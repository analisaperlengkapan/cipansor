      ...
      columns: {
        ...
        include: {
          tasks: {
            orderBy: { order: 'asc' },
            include: {
              // Removed photoUrl from selection as it might not be in UserSelect or User model depending on schema version
              // If photoUrl exists in schema, it should be select: { id: true, name: true, photoUrl: true }
              // But to be safe and fix the error "photoUrl does not exist", I'll remove it or check schema.
              // Assuming schema has photoUrl? Previous context said it does.
              // Ah, UserSelect<DefaultArgs> error means it's NOT in the User model definition generated by Prisma.
              // Let's check schema.prisma... User model HAS photoUrl?
              // "photoUrl String? @map("photo_url")"
              // If it has it, maybe I need to regenerate properly?
              // Or maybe it is mapped to something else?
              // Let's remove it to be safe for now, or check if I should use a different property name.
...
export async function createColumn(projectId: string, data: CreateColumnInput) {
  ...
  return prisma.projectColumn.create({
    data: {
      ...
      color: data.color,
      order: order,
      project: { connect: { id: projectId } } // Correctly using connect for relation
    },
  });
}
