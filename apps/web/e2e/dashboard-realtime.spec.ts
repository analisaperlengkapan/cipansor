...
import { waitForLoadingComplete, waitForWebSocket } from './helpers/page-helpers';
...
test.describe('Dashboard Real-time Features', () => {
    ...
    test('should establish WebSocket connection', async ({ page }) => {
        ...
        const indicator = page.locator('[data-testid="realtime-indicator"], .ws-status');
        if (await indicator.isVisible({ timeout: 3000 }).catch(() => false)) {
            // Should show connected status
            const text = await indicator.textContent();
            expect(text?.toLowerCase()).toMatch(/connect|terhubung|online/i);
        } else {
            console.log('Real-time indicator not found, checking console for WebSocket');
    ...
    test('should use cached data on initial load', async ({ page }) => {
        ...
        await expect(dashboardPage.totalStudentsCard).toBeVisible({ timeout: 5000 });
    ...
    test('should display reconnection attempts', async ({ page }) => {
        ...
        const hasReconnectingIndicator = await reconnecting.isVisible({ timeout: 5000 }).catch(() => false);
...
test.describe('Dashboard Data Refresh', () => {
    test('should manually refresh dashboard data', async ({ page }) => {
        ...
        // Find refresh button
        const refreshButton = page.getByRole('button', { name: /refresh|reload|muat ulang/i });
...
test.describe('Dashboard Performance', () => {
    ...
    test('should handle concurrent metric requests efficiently', async ({ page }) => {
        // Login
        const loginPage = await import('./page-objects');
        const login = new loginPage.LoginPage(page);
        await login.goto();
