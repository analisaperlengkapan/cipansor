...
export default function StaffAttendancePage() {
  ...
  const handleBulkAttendance = async () => {
    try {
      ...
      toast.info('Fitur Absensi Massal akan segera hadir. Silakan input manual per karyawan.');
      setIsBulkOpen(false);
    } catch (error) {
    ...
    <MainLayout>
      <div className="space-y-6">
        ...
        <div className="grid gap-4 md:grid-cols-4">
          ...
          <Card>
            <CardHeader className="flex flex-row items-center justify-between pb-2">
              <CardTitle className="text-sm font-medium">Absen (Alpha)</CardTitle>
                   ...
                   {isLoading ? (
                     ...
                     filteredAttendances.map((item) => (
                       <TableRow key={item.id}>
                         ...
                         <TableCell>
                           {item.checkIn ? format(new Date(item.checkIn), 'HH:mm') : '-'}
                         </TableCell>
                         <TableCell>
        ...
        <Dialog open={isBulkOpen} onOpenChange={setIsBulkOpen}>
           <DialogContent>
             ...
             <div className="py-4">
               <p className="text-muted-foreground">
                 Fitur ini akan memungkinkan Anda untuk menandai semua karyawan sebagai "Hadir" atau mengimpor data dari mesin fingerprint.
               </p>
