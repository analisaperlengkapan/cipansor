...
import { CalendarIcon, Save, ArrowLeft, Loader2, ImagePlus, X, CheckCircle2, Trash2 } from 'lucide-react';
...
export default function EditPAUDAssessmentPage() {
    const params = useParams();
    const router = useRouter();
    const id = params.id as string;
    const { user } = useAuthStore();

    const { data: assessment, isLoading: loadingAssessment } = usePAUDAssessment(id);
    ...
    const updateMutation = useUpdatePAUDAssessment();
    ...
    const onSubmit = async (values: FormValues) => {
        try {
            ...
            const message = error instanceof Error ? error.message : 'Gagal memperbarui penilaian';
            toast.error(message);
        }
    };
                                            ...
                                            <div className="space-y-3">
                                                ...
                                                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                                                    {existingEvidences.map((evidence) => (
                                                        <div key={evidence.id} className="relative aspect-square rounded-md overflow-hidden border group">
                                                            <img src={evidence.fileUrl} alt="Evidence" className="object-cover w-full h-full" />
                                                            <button
                                                                type="button"
                                                                onClick={() => deleteExistingEvidence(evidence.id)}
                                        ...
                                        <div className="space-y-3">
                                            ...
                                            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                                                ...
                                                <label className="flex flex-col items-center justify-center aspect-square rounded-md border-2 border-dashed border-muted-foreground/25 hover:border-primary hover:bg-accent/50 cursor-pointer transition-all">
                                                    ...
                                                    <span className="text-[10px] mt-2 font-medium">Tambah Foto</span>
                                                    <input
                                                        type="file"
                                                        multiple
                                                        accept="image/*"
                        ...
                        <div className="flex justify-between items-center bg-background/80 backdrop-blur-sm p-4 border rounded-lg sticky bottom-4 z-10">
                            ...
                            <div className="flex gap-3">
                                {step < 4 ? (
                                    ...
                                    <Button type="submit" disabled={updateMutation.isPending || addEvidenceMutation.isPending}>
