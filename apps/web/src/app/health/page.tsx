...
function QuickAdmitCard() {
  ...
  const commonAilments = [
    ...
    { label: "Flu / Batuk", icon: Activity, color: "text-blue-500 bg-blue-50 border-blue-200" },
    { label: "Sakit Kepala", icon: Activity, color: "text-orange-500 bg-orange-50 border-orange-200" },
    { label: "Luka Ringan", icon: Bandage, color: "text-amber-500 bg-amber-50 border-amber-200" },
    { label: "Maag", icon: Pill, color: "text-green-500 bg-green-50 border-green-200" },
  ];
  ...
  return (
    <Card className="h-full border-2 border-primary/20 shadow-md">
        ...
        <div className="space-y-2">
          ...
          <Popover open={open} onOpenChange={setOpen}>
            <PopoverTrigger asChild>
              ...
              >
                {selectedStudent ? selectedStudent.name : "Ketik nama santri..."}
            ...
            <PopoverContent className="w-[300px] p-0" align="start">
              <Command shouldFilter={false}>
                ...
                />
                <CommandList>
                    {isLoadingStudents && <div className="p-2 text-xs text-muted-foreground">Mencari...</div>}
                    {!isLoadingStudents && students?.length === 0 && <CommandEmpty>Tidak ditemukan.</CommandEmpty>}
                    <CommandGroup>
                    {students?.map((student) => (
                        ...
                        <div className="flex flex-col">
                            ...
                            <span className="text-xs text-muted-foreground">{student.nis} • {student.currentClass?.name || 'No Class'}</span>
                        </div>
                        </CommandItem>
                    ))}
                    </CommandGroup>
           ...
           <div className="grid grid-cols-2 gap-2">
              {commonAilments.map((item) => (
                 <Button
                    key={item.label}
                    variant="outline"
                    className={cn("h-auto py-3 justify-start gap-2 border", item.color, selectedStudent ? "opacity-100" : "opacity-50 cursor-not-allowed")}
                    onClick={() => selectedStudent && setConfirmComplaint(item.label)}
      ...
      {/* Confirmation Dialog */}
      <Dialog open={!!confirmComplaint} onOpenChange={(o) => !o && setConfirmComplaint(null)}>
        <DialogContent>
           <DialogHeader>
              <DialogTitle>Konfirmasi Quick Admit</DialogTitle>
              <DialogDescription>
                 Mencatat <strong>{confirmComplaint}</strong> untuk santri <strong>{selectedStudent?.name}</strong>?
              </DialogDescription>
           </DialogHeader>
           <DialogFooter>
              <Button variant="outline" onClick={() => setConfirmComplaint(null)}>Batal</Button>
              <Button onClick={handleQuickAdmit} disabled={createMutation.isPending}>
                 {createMutation.isPending ? "Menyimpan..." : "Konfirmasi & Simpan"}
              </Button>
           </DialogFooter>
        </DialogContent>
      </Dialog>
...
function SickBayMonitor({ summaryData }: { summaryData: any }) {
    ...
    const sickCount = summaryData?.recordsByType?.find((s: any) => s.type === 'ILLNESS')?.count || 0;
    ...
    return (
        <Card className="h-full bg-slate-900 text-white border-0 shadow-lg overflow-hidden relative">
            ...
            <CardHeader>
                ...
                <CardDescription className="text-slate-400">Real-time occupancy & status</CardDescription>
            </CardHeader>
            <CardContent className="space-y-6">
                <div>
                   <div className="flex justify-between text-sm mb-2">
                      <span className="text-slate-300">Bed Occupancy</span>
                      <span className="font-bold text-green-400">{sickCount} / {bedCapacity} Beds</span>
                   </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                    <div className="bg-slate-800 p-3 rounded-lg border border-slate-700">
                        <div className="text-xs text-slate-400 uppercase tracking-wider">Isolation Room</div>
                        <div className="text-2xl font-bold mt-1 text-yellow-400">0</div>
                        <div className="text-[10px] text-slate-500">Available</div>
                    </div>
                     <div className="bg-slate-800 p-3 rounded-lg border border-slate-700">
                        <div className="text-xs text-slate-400 uppercase tracking-wider">Doctor on Duty</div>
                        <div className="text-lg font-bold mt-1 text-white truncate">Dr. Aisyah</div>
    ...
    <div className="space-y-6 p-1">
      ...
      <div className="flex items-center justify-between pb-4 border-b">
        <div>
          ...
          <p className="text-muted-foreground">Pusat pemantauan kesehatan santri & manajemen UKS</p>
        </div>
            ...
            <div className="grid grid-cols-3 gap-4">
                <Card className="bg-gradient-to-br from-red-50 to-white border-red-100">
                    <CardHeader className="pb-2">
                        <CardTitle className="text-sm font-medium text-red-600">Sakit Hari Ini</CardTitle>
                    </CardHeader>
                    <CardContent>
                ...
                <Card className="bg-gradient-to-br from-blue-50 to-white border-blue-100">
                    <CardHeader className="pb-2">
                        <CardTitle className="text-sm font-medium text-blue-600">Obat Tersedia</CardTitle>
                    </CardHeader>
                    <CardContent>
                ...
                <Card className="bg-gradient-to-br from-yellow-50 to-white border-yellow-100">
                    <CardHeader className="pb-2">
                        <CardTitle className="text-sm font-medium text-yellow-600">Perlu Restock</CardTitle>
                    </CardHeader>
                    <CardContent>
      ...
      <div className="pt-8">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center justify-between mb-4">
          <h2 className="text-xl font-bold text-slate-800">Riwayat Medis Terkini</h2>
                ...
                <TableBody>
                    ...
                    <TableRow key={record.id} className="hover:bg-slate-50/50">
                        ...
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <p className="font-semibold text-slate-800">{record.student?.user?.name || record.student?.name}</p>
                                <p className="text-xs text-muted-foreground">{record.student?.nis}</p>
                            </div>
                        </div>
                        </TableCell>
                        <TableCell>
                           <Badge variant="secondary">{HEALTH_RECORD_TYPES.find(t => t.value === record.type)?.label}</Badge>
                        </TableCell>
                        <TableCell className="max-w-[200px]">
                            <div className="truncate font-medium text-slate-700">{record.diagnosis || record.complaint || '-'}</div>
                        </TableCell>
                        <TableCell>
                            <StatusBadge status={record.status || HealthStatus.HEALTHY} />
                        </TableCell>
