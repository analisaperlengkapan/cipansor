...
export async function getParentEngagementStats(
  ...
  try {
    const { unitId } = (req.query as any);
    const stats = await engagementService.getParentEngagementStats(unitId as string | undefined);
    res.json({ success: true, data: stats });
  } catch (error) {
    next(error);
  }
...
export async function getHomeroomPerformanceStats(
  req: Request,
  ...
  try {
    const { unitId } = (req.query as any);
    const stats = await engagementService.getHomeroomPerformanceStats(unitId as string | undefined);
    res.json({ success: true, data: stats });
  } catch (error) {
    next(error);
  }
