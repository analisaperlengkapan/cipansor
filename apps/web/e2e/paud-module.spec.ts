...
import { PAUDAssessmentPage } from './page-objects';
import { waitForLoadingComplete, waitForToast } from './helpers/page-helpers';

/**
 * PAUD Module E2E Tests
 * Tests PAUD assessment, development tracking, and reporting
 */

test.describe('PAUD Assessment', () => {
    let paudPage: PAUDAssessmentPage;
    ...
    test.beforeEach(async ({ page }) => {
        ...
        // Navigate to PAUD Assessment
        paudPage = new PAUDAssessmentPage(page);
        await paudPage.goto();
        await waitForLoadingComplete(page);
    });

    test('should display assessment page components', async ({ page }) => {
        await expect(paudPage.heading).toBeVisible({ timeout: 10000 });
        await expect(paudPage.studentSelect).toBeVisible();
        await expect(paudPage.aspectTabs).toBeVisible();
    });

    test('should display all development aspects', async ({ page }) => {
        // Standard PAUD aspects: Agama, Fisik, Kognitif, Bahasa, Sosial-Emosional, Seni
        const aspects = [
            /agama/i,
            /fisik/i,
            /kognitif/i,
    ...
    test('should select student and load indicators', async ({ page }) => {
        ...
        await paudPage.studentSelect.click();
    ...
    test('should save assessment successfully', async ({ page }) => {
        ...
        if (await checkboxes.count() > 0) {
            ...
            // Save
            await paudPage.saveButton.click();
...
test.describe('PAUD Reports', () => {
    test('should generate student development report', async ({ page }) => {
        ...
        if (await studentSelect.isVisible({ timeout: 5000 }).catch(() => false)) {
            ...
            test.skip(true, 'PAUD reports page not available');
        }
    });
});

test.describe('PAUD Dashboard', () => {
    test('should display PAUD dashboard with metrics', async ({ page }) => {
        // Login
        const loginPage = await import('./page-objects');
        const login = new loginPage.LoginPage(page);
        await login.goto();
        ...
        // Navigate to PAUD Dashboard
        await page.goto('/paud/dashboard');
        await waitForLoadingComplete(page);

        // Check for dashboard components
        const heading = page.getByRole('heading', { name: /dashboard paud/i });
        if (await heading.isVisible({ timeout: 5000 }).catch(() => false)) {
            ...
            test.skip(true, 'PAUD dashboard not available');
        }
    });
});
