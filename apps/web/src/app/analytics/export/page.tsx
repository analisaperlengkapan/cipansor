...
export default function ExportPage() {
    ...
    const { data: previewData, isLoading: previewLoading, refetch } = useQuery({
        ...
        queryFn: () => {
            ...
            return api.get(selectedExportType?.endpoint || '', { params }).then(res => res.data);
        },
    ...
    const exportMutation = useMutation({
        mutationFn: async () => {
            ...
            if (format === 'csv') {
                ...
                link.href = url;
                link.setAttribute('download', `${selectedType}_export_${new Date().toISOString().split('T')[0]}.csv`);
                document.body.appendChild(link);
                link.click();
                link.remove();
                window.URL.revokeObjectURL(url);
            } else {
                const response = await api.get(selectedExportType?.endpoint || '', { params });
                // Download as JSON
                const dataStr = JSON.stringify(response.data, null, 2);
                const dataUri = 'data:application/json;charset=utf-8,' + encodeURIComponent(dataStr);
                const link = document.createElement('a');
                link.href = dataUri;
                link.setAttribute('download', `${selectedType}_export_${new Date().toISOString().split('T')[0]}.json`);
                document.body.appendChild(link);
                link.click();
                link.remove();
            }
                    ...
                    <CardContent className="space-y-2">
                        {EXPORT_TYPES.map((type) => (
                            <div
                                ...
                                className={`flex items-center gap-3 p-3 rounded-lg cursor-pointer transition-colors ${selectedType === type.id
                ...
                <Card className="lg:col-span-2">
                    ...
                    <CardContent className="space-y-4">
                        <div className="grid gap-4 md:grid-cols-2">
                            ...
                            {(selectedType === 'attendance' || selectedType === 'finance' || selectedType === 'tahfidz') && (
                                <>
                                    <div className="space-y-2">
                        ...
                        <div className="flex gap-2">
                            <Button onClick={() => refetch()} variant="outline" disabled={previewLoading}>
                            ...
                            <div className="mt-4">
                                <div className="flex items-center justify-between mb-2">
                                    <p className="text-sm text-muted-foreground">
                                        Preview data ({Math.min(5, previewData.data.length)} dari {previewData.count || previewData.data.length} records)
                                    </p>
                                    <Badge variant="secondary">{previewData.count || previewData.data.length} total</Badge>
                                </div>
                                <div className="border rounded-lg overflow-auto">
                                    <Table>
                                        ...
                                        <TableBody>
                                            {previewData.data.slice(0, 5).map((row: Record<string, unknown>, i: number) => (
                                                <TableRow key={i}>
                                                    {getPreviewColumns().map((col) => (
