...
test.describe("Landing Page", () => {
  ...
  test("should display hero section with main title", async ({ page }) => {
    await expect(page.getByRole("heading", { name: "Membangun Generasi Qur'ani" })).toBeVisible();
    // Scope to hero section
    const hero = page.locator('#hero');
    await expect(hero.getByRole("link", { name: "Daftar Sekarang" })).toBeVisible();
  });
  ...
  test("should display programs section", async ({ page }) => {
    ...
    await expect(programs.getByRole("heading", { name: "Jenjang Pendidikan" })).toBeVisible();
