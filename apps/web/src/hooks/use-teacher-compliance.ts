...
export type CertificationStatus =
  (typeof CERTIFICATION_STATUS)[number]["value"];
...
export function useTeacherComplianceList(params?: UseTeacherComplianceParams) {
  return useQuery({
    ...
    queryFn: async () => {
      const response = await api.get<ApiResponse<TeacherComplianceData[]>>(
        "/hr/teachers",
        {
          params: {
            ...params,
...
export function useUpdateTeacherCompliance() {
  ...
  return useMutation({
    mutationFn: async ({
      ...
      teacherId: string;
      data: UpdateTeacherComplianceData;
    }) => {
      const response = await api.put<ApiResponse<TeacherComplianceData>>(
        `/teacher-compliance/${teacherId}`,
        data,
      );
      return response.data.data;
    ...
    onSuccess: (_, variables) => {
      queryClient.invalidateQueries({
        queryKey: ["teacher-compliance", variables.teacherId],
      });
      queryClient.invalidateQueries({
        queryKey: ["teacher-compliance", "list"],
      });
      queryClient.invalidateQueries({
        queryKey: ["teacher-compliance", "report"],
      });
    },
  });
}
...
export function useBulkUpdateTeacherCompliance() {
  ...
  return useMutation({
    mutationFn: async (
      updates: { teacherId: string; data: UpdateTeacherComplianceData }[],
    ) => {
      const response = await api.post<ApiResponse<{ updated: number }>>(
        "/teacher-compliance/bulk-update",
        {
          updates,
        },
      );
...
export function useTeacherComplianceReport(unitId?: string) {
  return useQuery({
    ...
    queryFn: async () => {
      const response = await api.get<ApiResponse<TeacherComplianceReport>>(
        "/teacher-compliance/report/completeness",
        {
...
export function useSimtunReadyReport(unitId?: string) {
  return useQuery({
    ...
    queryFn: async () => {
      const response = await api.get<ApiResponse<SimtunReadyReport>>(
        "/teacher-compliance/report/simtun-ready",
        {
          params: unitId ? { unitId } : undefined,
        },
      );
...
export function useCertificationReport(unitId?: string) {
  return useQuery({
    ...
    queryFn: async () => {
      const response = await api.get<ApiResponse<CertificationReport>>(
        "/teacher-compliance/report/certification",
        {
          params: unitId ? { unitId } : undefined,
        },
      );
