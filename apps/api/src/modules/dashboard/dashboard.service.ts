...
export class DashboardService {
    ...
    async getTahfidzStats(
        ...
        const averageJuz = totalStudentsWithRecords > 0
    ...
    async getViolationRewardStats(
        ...
        const [totalViolations, totalRewards, recentViolationsRaw, recentRewardsRaw] = await Promise.all([
    ...
    private async getRecentMetricsHistory(unitId?: string): Promise<DashboardMetrics[]> {
        try {
            ...
            return history
                .map(h => h.metrics as unknown as DashboardMetrics)
                .reverse();
        } catch (error) {
    ...
    private async getActiveAlerts(unitId?: string): Promise<DashboardAlert[]> {
        ...
        try {
            ...
            const inactivityRate = totalActiveStudents > 0
                ? (inactiveStudents / totalActiveStudents) * 100
                : 0;
