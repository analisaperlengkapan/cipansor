...
import { waitForLoadingComplete, waitForToast, navigateTo } from './helpers/page-helpers';
...
test.describe('Integration: Student → Attendance → Report Flow', () => {
  test('complete workflow: create student, mark attendance, view report', async ({ page }) => {
    ...
    await page.getByLabel(/nama|name/i).first().fill(studentName);
    await page.getByLabel(/nisn|nis/i).first().fill(studentNISN);
    ...
    const classSelect = page.locator('button[role="combobox"]').filter({ hasText: /kelas|class/i }).first();
    ...
    const attendanceClassSelect = page.locator('button[role="combobox"]').filter({ hasText: /kelas|class/i }).first();
    if (await attendanceClassSelect.isVisible({ timeout: 5000 }).catch(() => false)) {
      await attendanceClassSelect.click();
      await page.getByRole('option').first().click();
      await waitForLoadingComplete(page);
      
      // Find our student in the list
      const studentRow = page.locator('table tbody tr, [role="listitem"]').filter({ hasText: studentName });
      
      if (await studentRow.isVisible({ timeout: 3000 }).catch(() => false)) {
        // Mark as present
        const presentButton = studentRow.getByRole('button', { name: /hadir|present/i }).or(
          studentRow.locator('input[type="radio"][value="PRESENT"]')
        );
        
        if (await presentButton.isVisible({ timeout: 2000 }).catch(() => false)) {
          await presentButton.click();
          
          const saveAttendance = page.getByRole('button', { name: /simpan|save/i });
          if (await saveAttendance.isVisible({ timeout: 2000 }).catch(() => false)) {
    ...
    const reportHeading = page.getByRole('heading', { name: /laporan|report/i });
    if (await reportHeading.isVisible({ timeout: 5000 }).catch(() => false)) {
      // Generate report
      const generateButton = page.getByRole('button', { name: /generate|tampilkan|buat/i });
    ...
    if (await searchInput.isVisible({ timeout: 3000 }).catch(() => false)) {
      ...
      if (await deleteButton.isVisible({ timeout: 3000 }).catch(() => false)) {
        ...
        const confirmButton = page.getByRole('button', { name: /ya|yes|confirm|hapus/i });
...
test.describe('Integration: Dashboard → Module Navigation', () => {
  test('should navigate from dashboard to modules and back', async ({ page }) => {
    const login = new LoginPage(page);
    await login.goto();
    await login.loginAndWaitForDashboard('superadmin@cipansor.id', 'SuperAdmin123!');
    ...
    const studentsLink = page.getByRole('link', { name: /santri|students|siswa/i });
    ...
    const attendanceLink = page.getByRole('link', { name: /absen|attendance|kehadiran/i });
...
test.describe('Integration: Student → Tahfidz Progress', () => {
  test('should track tahfidz progress for student', async ({ page }) => {
    ...
    if (await firstStudent.isVisible({ timeout: 5000 }).catch(() => false)) {
      ...
      const viewButton = firstStudent.getByRole('button', { name: /view|lihat|detail/i });
      ...
      if (await tahfidzSection.isVisible({ timeout: 3000 }).catch(() => false)) {
        await expect(tahfidzSection).toBeVisible();
        
        // Should show progress indicators
        const hasProgress = await page.locator('[role="progressbar"], .progress').isVisible({ timeout: 2000 }).catch(() => false);
...
test.describe('Integration: PAUD Assessment → Report Generation', () => {
  test('complete PAUD assessment and generate report', async ({ page }) => {
    ...
    const heading = page.getByRole('heading', { name: /penilaian|assessment/i });
    if (await heading.isVisible({ timeout: 5000 }).catch(() => false)) {
      // Select student
      const studentSelect = page.locator('button[role="combobox"]').filter({ hasText: /santri|student/i }).first();
      if (await studentSelect.isVisible({ timeout: 3000 }).catch(() => false)) {
        ...
        if (optionCount > 0) {
          ...
          if (checkboxCount > 0) {
            ...
            if (await levelRadio.isVisible({ timeout: 2000 }).catch(() => false)) {
            ...
            const reportStudentSelect = page.locator('button[role="combobox"]').filter({ hasText: /santri|student/i }).first();
            if (await reportStudentSelect.isVisible({ timeout: 3000 }).catch(() => false)) {
              await reportStudentSelect.click();
              
              const reportOption = page.getByRole('option').filter({ hasText: selectedStudent || /./i }).first();
              if (await reportOption.isVisible({ timeout: 2000 }).catch(() => false)) {
                await reportOption.click();
                
                // Generate report
                const generateButton = page.getByRole('button', { name: /generate|buat|tampilkan/i });
                if (await generateButton.isVisible({ timeout: 3000 }).catch(() => false)) {
                  await generateButton.click();
                  await waitForLoadingComplete(page);
                  ...
                  await expect(reportContent.or(page.locator('table'))).toBeVisible({ timeout: 5000 });
                }
              }
            }
          }
...
test.describe('Integration: Finance → Dashboard Sync', () => {
  test('should sync finance data with dashboard', async ({ page }) => {
    ...
    if (await financeCard.isVisible({ timeout: 5000 }).catch(() => false)) {
      ...
      await expect(page.getByRole('heading', { name: /keuangan|finance/i })).toBeVisible({ timeout: 5000 });
...
test.describe('Integration: Multi-user Collaboration', () => {
  test('should handle concurrent user actions', async ({ page, context }) => {
    ...
    await login2.goto();
    await login2.loginAndWaitForDashboard('superadmin@cipansor.id', 'SuperAdmin123!');
