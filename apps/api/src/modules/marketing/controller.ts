...
import { calculateCampaignROI, getFunnelEfficiency } from './roi.service';
import {
...
export const getFunnelEfficiency = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const { unitId } = (req.query as any);
    const stats = await getFunnelEfficiency(unitId as string);
