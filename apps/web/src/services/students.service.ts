...
export type StudentStatus = 'ACTIVE' | 'INACTIVE' | 'GRADUATED' | 'DROPPED_OUT' | 'TRANSFERRED';
...
export interface ListStudentParams extends PaginationParams, UnitFilterParams, SortParams {
    classId?: string;
    dormitoryId?: string;
    status?: StudentStatus;
    gender?: Gender;
...
export const studentsService = {
    ...
    async getById(id: string): Promise<StudentDetail> {
        const response = await api.get<ApiResponse<StudentDetail>>(`/students/${id}`);
    ...
    async getByNis(nis: string): Promise<StudentDetail> {
        const response = await api.get<ApiResponse<StudentDetail>>(`/students/nis/${nis}`);
        return response.data.data;
    },
    ...
    async update(id: string, input: UpdateStudentInput): Promise<Student> {
        const response = await api.patch<ApiResponse<Student>>(`/students/${id}`, input);
        return response.data.data;
    },
     ...
     */
    async assignToDormitory(studentId: string, dormitoryId: string): Promise<Student> {
        const response = await api.post<ApiResponse<Student>>(
    ...
    async getStatistics(params?: UnitFilterParams): Promise<StudentStatistics> {
        const response = await api.get<ApiResponse<StudentStatistics>>('/students/statistics', {
            params
        });
        return response.data.data;
    },
     ...
     */
    async uploadPhoto(studentId: string, file: File): Promise<{ photoUrl: string }> {
     ...
     */
    async importStudents(file: File, unitId: string): Promise<{
        imported: number;
        failed: number;
     ...
     */
    async export(params?: ListStudentParams & { format?: 'csv' | 'xlsx' }): Promise<Blob> {
     ...
     */
    async transfer(studentId: string, newUnitId: string, reason?: string): Promise<Student> {
        const response = await api.post<ApiResponse<Student>>(
