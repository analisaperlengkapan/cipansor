...
test.describe('Finance - Payment Management', () => {
  ...
  test('should display payment list', async ({ page }) => {
    ...
    if (await heading.isVisible({ timeout: 5000 }).catch(() => false)) {
      ...
      // Verify headers
      const expectedHeaders = ['siswa|student', 'jumlah|amount', 'tanggal|date', 'status'];
  ...
  test('should create new payment', async ({ page }) => {
    ...
    if (await heading.isVisible({ timeout: 5000 }).catch(() => false)) {
      ...
      if (await addButton.isVisible({ timeout: 3000 }).catch(() => false)) {
        ...
        // Select student
        const studentSelect = page.locator('button[role="combobox"]').filter({ hasText: /siswa|student/i }).first();
        ...
        const methodSelect = page.locator('button[role="combobox"]').filter({ hasText: /metode|method/i }).first();
  ...
  test('should filter payments by status', async ({ page }) => {
    ...
    if (await heading.isVisible({ timeout: 5000 }).catch(() => false)) {
      ...
      if (await filterButton.isVisible({ timeout: 3000 }).catch(() => false)) {
        ...
        const statusOption = page.getByRole('option', { name: /lunas|paid/i }).or(
          page.getByRole('menuitem', { name: /lunas|paid/i })
        );
...
test.describe('Finance - Invoice Management', () => {
  ...
  test('should generate bulk invoices', async ({ page }) => {
    ...
    const heading = page.getByRole('heading', { name: /tagihan|invoice/i });
    if (await heading.isVisible({ timeout: 5000 }).catch(() => false)) {
      const generateButton = page.getByRole('button', { name: /generate|buat.*tagihan/i });
      if (await generateButton.isVisible({ timeout: 3000 }).catch(() => false)) {
        ...
        if (await classSelect.isVisible({ timeout: 3000 }).catch(() => false)) {
          ...
          const confirmButton = page.getByRole('button', { name: /generate|buat/i });
  ...
  test('should view invoice detail', async ({ page }) => {
    ...
    if (await heading.isVisible({ timeout: 5000 }).catch(() => false)) {
      ...
      if (await firstRow.isVisible({ timeout: 3000 }).catch(() => false)) {
        ...
        const detailContent = page.getByText(/detail.*tagihan|invoice.*detail/i);
...
test.describe('Finance - Reports', () => {
  ...
  test('should generate income report', async ({ page }) => {
    ...
    if (await heading.isVisible({ timeout: 5000 }).catch(() => false)) {
      // Select report type
      const reportTypeSelect = page.locator('button[role="combobox"]').filter({ hasText: /jenis.*laporan|report.*type/i }).first();
      if (await reportTypeSelect.isVisible({ timeout: 3000 }).catch(() => false)) {
      ...
      // Generate report
      const generateButton = page.getByRole('button', { name: /generate|tampilkan/i });
      await generateButton.click();
      await waitForLoadingComplete(page);
...
test.describe('Finance - Dashboard Integration', () => {
  ...
  test('should display finance widgets on main dashboard', async ({ page }) => {
    ...
    const financeWidget = page.getByText(/keuangan|finance|pendapatan|revenue/i).first();
    if (await financeWidget.isVisible({ timeout: 5000 }).catch(() => false)) {
      ...
      const metrics = page.locator('[data-testid*="finance"], .finance-card, .revenue-card');
...
test.describe('Finance - Payment Reminder', () => {
  ...
  test('should send payment reminder', async ({ page }) => {
    ...
    const heading = page.getByRole('heading', { name: /pengingat|reminder/i });
    if (await heading.isVisible({ timeout: 5000 }).catch(() => false)) {
      const sendButton = page.getByRole('button', { name: /kirim.*pengingat|send.*reminder/i }).first();
      if (await sendButton.isVisible({ timeout: 3000 }).catch(() => false)) {
        await sendButton.click();
        
        const confirmButton = page.getByRole('button', { name: /ya|yes|kirim/i });
        await confirmButton.click();
