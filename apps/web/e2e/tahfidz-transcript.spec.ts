...
test.describe('Tahfidz Transcript', () => {
    test.beforeEach(async ({ page }) => {
        ...
        await page.route('**/api/auth/login', async route => {
            const json = {
                ...
                data: {
                    ...
                    user: { id: 'super-admin-id', name: 'Super Admin', role: 'SUPER_ADMIN' }
                }
            };
        ...
        await page.route('**/api/auth/me', async route => {
            const json = {
                success: true,
                data: { id: 'super-admin-id', name: 'Super Admin', role: 'SUPER_ADMIN' }
            };
            await route.fulfill({ json });
        });
        ...
        await page.route('**/api/units*', async route => {
            await route.fulfill({ json: { success: true, data: [{ id: 'unit-1', name: 'SMP IT' }] } });
        });
        await page.route('**/api/classes*', async route => {
            await route.fulfill({ json: { success: true, data: [{ id: 'class-1', name: '7A' }] } });
        });
        await page.route('**/api/academic-years*', async route => {
            await route.fulfill({ json: { success: true, data: [{ id: 'ay-1', name: '2024/2025', isActive: true }] } });
        });
    ...
    test('should display tahfidz summary with correct types', async ({ page }) => {
        ...
        await page.route('**/api/tahfidz?*', async route => {
             await route.fulfill({ json: { success: true, data: [], meta: { pagination: { total: 0 } } } });
        });
        ...
        await page.screenshot({ path: '/home/jules/verification/transcript_verified.png' });
    });
});
