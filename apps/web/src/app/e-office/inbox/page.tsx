      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-4">
        <Card className="cursor-pointer hover:bg-muted/50" onClick={() => setDirection(LetterDirection.INCOMING)}>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            ...
            <Mail className={`h-4 w-4 ${direction === LetterDirection.INCOMING ? 'text-primary' : 'text-muted-foreground'}`} />
          </CardHeader>
          <CardContent>
            <div className="text-2xl font-bold">Inbox</div>
          </CardContent>
        </Card>
        <Card className="cursor-pointer hover:bg-muted/50" onClick={() => setDirection(LetterDirection.OUTGOING)}>
          <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
            <CardTitle className="text-sm font-medium">Surat Keluar</CardTitle>
            <Send className={`h-4 w-4 ${direction === LetterDirection.OUTGOING ? 'text-primary' : 'text-muted-foreground'}`} />
          </CardHeader>
          <CardContent>
          ...
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <TableHead>{direction === LetterDirection.INCOMING ? 'Pengirim' : 'Tujuan'}</TableHead>
                  ...
                  >
                    ...
                    <TableCell>
                      <div className="flex flex-col">
                        ...
                        <span className="text-xs text-muted-foreground line-clamp-1">{letter.classification?.code} - {letter.classification?.name}</span>
                      </div>
                    </TableCell>
                    <TableCell>
                    ...
                    <TableCell>
                      <span className={`text-xs px-2 py-1 rounded-full border ${
                        letter.urgency === 'URGENT' ? 'bg-red-50 border-red-200 text-red-700' :
                        letter.urgency === 'IMMEDIATE' ? 'bg-orange-50 border-orange-200 text-orange-700' :
