...
export default function ProcurementDetailPage() {
  ...
  if (!request) return <MainLayout><div>Data not found</div></MainLayout>;

  const canApprove = ['SUPER_ADMIN', 'UNIT_ADMIN', 'YAYASAN_ADMIN'].includes(user?.role || '') && request.status === PurchaseRequestStatus.PENDING;
  const canFulfill = ['SUPER_ADMIN', 'UNIT_ADMIN', 'YAYASAN_ADMIN'].includes(user?.role || '') && (request.status === PurchaseRequestStatus.APPROVED || request.status === PurchaseRequestStatus.ORDERED);
    ...
    <MainLayout>
      <div className="space-y-6">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-bold tracking-tight">Detail Pengajuan: {request.code}</h1>
            <p className="text-muted-foreground">
              Dibuat oleh {request.requester?.name} pada {format(new Date(request.date), 'dd MMMM yyyy', { locale: idLocale })}
            </p>
          </div>
        </div>
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                <div className="flex justify-between items-center p-4 bg-muted/50 rounded-lg">
                  ...
                  <Badge className="text-base px-4 py-1" variant={request.status === 'REJECTED' ? 'destructive' : 'default'}>
                    {request.status}
                  </Badge>
                </div>
                ...
                {request.rejectionReason && (
                   <div className="p-4 bg-red-50 text-red-800 rounded-lg border border-red-200">
                     <p className="font-semibold flex items-center gap-2"><XCircle className="h-4 w-4"/> Alasan Penolakan:</p>
                ...
                {request.status === 'PENDING' && (
                  <div className="p-4 bg-yellow-50 text-yellow-800 rounded-lg border border-yellow-200">
                     <p className="font-semibold flex items-center gap-2"><AlertTriangle className="h-4 w-4"/> Menunggu Persetujuan</p>
                     <p>Mohon periksa anggaran sebelum menyetujui pengajuan ini.</p>
                  </div>
                )}
                    ...
                    <>
                      <Dialog open={isRejectDialogOpen} onOpenChange={setIsRejectDialogOpen}>
                        ...
                        <DialogContent>
                          ...
                          />
                          <DialogFooter>
                            <Button onClick={handleReject} disabled={isUpdating}>Tolak Pengajuan</Button>
                          </DialogFooter>
                        </DialogContent>
                      </Dialog>
             ...
             <Card>
               ...
               <CardContent className="space-y-4 text-sm">
                 ...
                 <div>
                   ...
                   <p className="font-medium">{format(new Date(request.date), 'dd MMM yyyy', { locale: idLocale })}</p>
                 </div>
                 <div>
                 ...
                 {request.approvedBy && (
                   <div className="pt-4 border-t">
                     ...
                     <p className="text-xs text-muted-foreground">{format(new Date(request.approvedAt!), 'dd MMM yyyy HH:mm')}</p>
                   </div>
                 )}
                 {request.receivedAt && (
                   <div className="pt-4 border-t text-green-700">
                     <p className="font-medium flex items-center gap-2"><CheckCircle className="h-3 w-3"/> Barang Diterima</p>
