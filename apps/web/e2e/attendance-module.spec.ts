...
import { waitForLoadingComplete, waitForToast, fillForm } from './helpers/page-helpers';
...
test.describe('Attendance - Daily Marking', () => {
  ...
  test('should display attendance page', async ({ page }) => {
    await expect(page.getByRole('heading', { name: /absen|attendance|kehadiran/i })).toBeVisible({ timeout: 10000 });
    
    // Check for date selector
    const dateInput = page.locator('input[type="date"]').or(page.getByLabel(/tanggal|date/i));
    ...
    if (await classSelect.isVisible({ timeout: 5000 }).catch(() => false)) {
      ...
      if (optionCount > 0) {
        ...
        if (studentCount > 0) {
          ...
          const presentButton = studentList.first().getByRole('button', { name: /hadir|present/i }).or(
            studentList.first().locator('input[type="radio"][value="PRESENT"]')
          );
  ...
  test('should mark student as absent with reason', async ({ page }) => {
    ...
    if (await classSelect.isVisible({ timeout: 5000 }).catch(() => false)) {
      ...
      if (studentCount > 0) {
        // Mark as absent
        const absentButton = studentList.first().getByRole('button', { name: /tidak hadir|absent/i }).or(
          studentList.first().locator('input[type="radio"][value="ABSENT"]')
        );
        
        if (await absentButton.isVisible({ timeout: 3000 }).catch(() => false)) {
  ...
  test('should mark all students present (bulk action)', async ({ page }) => {
    ...
    if (await classSelect.isVisible({ timeout: 5000 }).catch(() => false)) {
      ...
      const markAllButton = page.getByRole('button', { name: /semua hadir|mark all present/i });
...
test.describe('Attendance - View Records', () => {
  ...
  test('should view attendance summary statistics', async ({ page }) => {
    ...
    for (const pattern of summaryCards) {
      if (await page.getByText(pattern).isVisible({ timeout: 3000 }).catch(() => false)) {
        foundCards++;
      }
    }
...
test.describe('Attendance - Reports', () => {
  test('should generate attendance report', async ({ page }) => {
    ...
    if (await heading.isVisible({ timeout: 5000 }).catch(() => false)) {
      // Select report type
      const reportType = page.locator('button[role="combobox"]').filter({ hasText: /jenis|type/i }).first();
      ...
      if (await reportType.isVisible({ timeout: 3000 }).catch(() => false)) {
        ...
        const hasReport = await page.locator('table, canvas, .report-content').isVisible({ timeout: 5000 }).catch(() => false);
