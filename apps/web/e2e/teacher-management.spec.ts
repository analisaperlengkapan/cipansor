...
test.describe('Teacher Management - List and View', () => {
  ...
  test('should view teacher detail', async ({ page }) => {
    ...
    if (await heading.isVisible({ timeout: 5000 }).catch(() => false)) {
      ...
      const detailHeading = page.getByRole('heading', { name: new RegExp(teacherName || 'detail', 'i') });
  ...
  test('should search teachers', async ({ page }) => {
    ...
    if (await heading.isVisible({ timeout: 5000 }).catch(() => false)) {
      ...
      const teacherName = (await teacherNameCell.textContent())?.trim().split(' ')[0];
      ...
      if (teacherName) {
        ...
        const firstResultName = await rows.first().locator('td').first().textContent();
        expect(firstResultName?.toLowerCase()).toContain(teacherName.toLowerCase());
      }
    } else {
      test.skip(true, 'Teachers page not available');
    }
  ...
  test('should filter teachers by subject', async ({ page }) => {
    ...
    if (await heading.isVisible({ timeout: 5000 }).catch(() => false)) {
      // Open subject filter
      const filterButton = page.getByRole('button', { name: /filter|mata pelajaran|subject/i });
      if (await filterButton.isVisible({ timeout: 3000 }).catch(() => false)) {
        await filterButton.click();
        
        // Select a subject
        const subjectOptions = page.getByRole('option').or(page.getByRole('menuitem'));
        ...
        if (optionCount > 0) {
          ...
          if (rowCount > 0) {
            ...
            const filterIndicator = page.getByText(new RegExp(selectedSubject || 'filter', 'i'));
...
test.describe('Teacher Management - Create', () => {
  test('should create new teacher successfully', async ({ page }) => {
    ...
    if (await heading.isVisible({ timeout: 5000 }).catch(() => false)) {
      ...
      // Select gender
      const genderSelect = page.locator('button[role="combobox"]').filter({ hasText: /jenis kelamin|gender/i }).first();
      ...
      await expect(page.getByRole('heading', { name: /guru|teacher/i })).toBeVisible({ timeout: 5000 });
      ...
      const newTeacherRow = page.locator('table tbody tr').filter({ hasText: `NIP${timestamp.toString().slice(-10)}` });
...
test.describe('Teacher Management - Update', () => {
  test('should update teacher information', async ({ page }) => {
    ...
    if (await heading.isVisible({ timeout: 5000 }).catch(() => false)) {
      ...
      // Verify changes
      const updatedRow = page.locator('table tbody tr').filter({ hasText: newPhone });
...
test.describe('Teacher Management - Delete', () => {
  test('should delete teacher with confirmation', async ({ page }) => {
    ...
    if (await heading.isVisible({ timeout: 5000 }).catch(() => false)) {
      ...
      const teacherRow = page.locator('table tbody tr').filter({ hasText: teacherNIP });
      const deleteButton = teacherRow.getByRole('button', { name: /delete|hapus/i });
      ...
      if (await deleteButton.isVisible({ timeout: 3000 }).catch(() => false)) {
        ...
        const noDataMessage = page.getByText(/tidak.*ditemukan|no.*found|kosong/i);
...
test.describe('Teacher Management - Subject Assignment', () => {
  test('should assign subjects to teacher', async ({ page }) => {
    ...
    if (await heading.isVisible({ timeout: 5000 }).catch(() => false)) {
      ...
      const actionButton = firstRow.getByRole('button', { name: /view|lihat|edit/i }).first();
      await actionButton.click();
      await waitForLoadingComplete(page);
      
      // Look for subject assignment section
      const subjectSection = page.getByText(/mata pelajaran|subject|mengajar/i);
      if (await subjectSection.isVisible({ timeout: 3000 }).catch(() => false)) {
        // Click add subject button
        const addSubjectButton = page.getByRole('button', { name: /tambah.*pelajaran|add.*subject/i });
  ...
  test('should assign teacher to class', async ({ page }) => {
    ...
    if (await heading.isVisible({ timeout: 5000 }).catch(() => false)) {
      ...
      if (await classSection.isVisible({ timeout: 3000 }).catch(() => false)) {
        const assignButton = page.getByRole('button', { name: /assign|tetapkan|pilih.*kelas/i });
