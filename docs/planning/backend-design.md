...
class PAUDAssessmentService {
   ...
   */
  private calculateTrend(assessments: PAUDDevelopmentAssessment[]): 'improving' | 'stable' | 'declining' {
    ...
    const avgFirst = this.average(firstHalf.map(a => levelValues[a.achievementLevel]));
    const avgSecond = this.average(secondHalf.map(a => levelValues[a.achievementLevel]));
...
class DailyReportService {
    ...
    for (const parent of parents) {
      if (via === 'whatsapp' || via === 'all') {
        if (parent.whatsappNumber) {
          results.push(await this.whatsappService.send(
      ...
      if (via === 'email' || via === 'all') {
        if (parent.email) {
          results.push(await this.emailService.send(
            parent.email,
   ...
   */
  private buildNotificationMessage(report: DailyStudentReport): NotificationMessage {
    const unitTemplates = {
  ...
  private buildPAUDMessage(report: DailyStudentReport): NotificationMessage {
    const moodEmoji = { HAPPY: '😊', NEUTRAL: '😐', SAD: '😢', TIRED: '😴', EXCITED: '🤩', SICK: '🤒' };
...
class DashboardSnapshotService {
  ...
  async calculateDailySnapshot(): Promise<void> {
    ...
    for (const unit of units) {
      // Student count
      await this.saveMetric(unit.id, 'STUDENT_COUNT', 
        await this.countStudents(unit.id), 'DAILY', today);
      
      // Attendance rate
      await this.saveMetric(unit.id, 'ATTENDANCE_RATE', 
        await this.calculateAttendanceRate(unit.id, today), 'DAILY', today);
      
      // Tahfidz progress (for relevant units)
      if (['SMP_IT', 'SMA_QURAN', 'PESANTREN'].includes(unit.type)) {
        await this.saveMetric(unit.id, 'TAHFIDZ_PROGRESS', 
          await this.calculateTahfidzProgress(unit.id), 'DAILY', today);
      }
    }
    
    // Yayasan-level (all units)
    await this.saveMetric(null, 'TOTAL_STUDENTS', 
      await this.countStudents(null), 'DAILY', today);
    await this.saveMetric(null, 'OVERALL_ATTENDANCE', 
      await this.calculateAttendanceRate(null, today), 'DAILY', today);
  }
...
const MODULE_PERMISSIONS = {
  'paud-assessment': {
    create: ['PAUD_GURU', 'PAUD_PENDAMPING', 'PAUD_KEPALA_SEKOLAH', 'PAUD_ADMIN'],
    read: ['PAUD_GURU', 'PAUD_PENDAMPING', 'PAUD_KEPALA_SEKOLAH', 'PAUD_ADMIN', 'PAUD_ORANG_TUA'],
  ...
  'murojaah': {
    ...
    read: ['MUHAFIDZ', 'MUSYRIF', '*_GURU', '*_ADMIN', '*_ORANG_TUA', '*_SISWA'],
    update: ['MUHAFIDZ'],
  ...
  'yayasan-dashboard': {
    read: ['YAYASAN_ADMIN', 'YAYASAN_KETUA', 'YAYASAN_SEKRETARIS', 'YAYASAN_BENDAHARA', 'SUPER_ADMIN']
  }
};
```
...
class WhatsAppService {
  ...
  async sendDailyReport(
    ...
    const templateName = report.unitType === 'SD_IT' 
...
| reportGenerationJob | 0 0 1 */6 * | Auto-generate semester reports |
