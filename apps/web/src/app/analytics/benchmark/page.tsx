...
export default function BenchmarkPage() {
    ...
    const { data: summary, isLoading: loadingSummary } = useQuery<BenchmarkSummary>({
    ...
    const { data: comparison, isLoading: loadingComparison } = useQuery<{
        ...
        queryFn: async () => {
            const res = await axios.get(`${API_BASE}/api/analytics/benchmark/compare`, {
                headers: { Authorization: `Bearer ${localStorage.getItem('token')}` },
            });
            return res.data.data;
        },
    ...
    // Fetch rankings
    const { data: rankings, isLoading: loadingRankings } = useQuery<RankingResult[]>({
        queryKey: ['benchmark-rankings', selectedMetric],
        queryFn: async () => {
            const res = await axios.get(`${API_BASE}/api/analytics/benchmark/rankings`, {
    ...
    // Prepare chart data
    const barChartData = comparison?.units.map((unit) => ({
        name: unit.unitName,
    ...
    const radarData = comparison?.units.map((unit) => ({
                        ...
                        <div className="grid gap-4 md:grid-cols-4">
                            ...
                            <div className="text-center">
                                <p className="text-3xl font-bold text-blue-600">
                                    {summary?.overallAverages.paymentCollectionRate.toFixed(1) || 0}%
                                </p>
                                ...
                                {loadingComparison ? (
                                    ...
                                    <ResponsiveContainer width="100%" height={400}>
                                        <BarChart data={barChartData}>
                                            ...
                                            <Tooltip />
                                            <Legend />
                                            <Bar dataKey="Kehadiran" fill={METRIC_COLORS.attendance} />
                                            ...
                                            {rankings?.map((item, idx) => (
                                                <TableRow key={`${item.unitId}-${item.metric}-${idx}`}>
                                                    ...
                                                    <TableCell>
                                                        <Badge
                                                            variant="outline"
                                                            style={{ borderColor: METRIC_COLORS[item.metric] }}
                                                        >
