...
import { waitForLoadingComplete, waitForToast, navigateTo, fillForm } from './helpers/page-helpers';
...
test.describe('Class Management - List and View', () => {
  ...
  test('should display class list', async ({ page }) => {
    ...
    if (await heading.isVisible({ timeout: 5000 }).catch(() => false)) {
      ...
      const expectedHeaders = ['nama.*kelas|class.*name', 'tingkat|grade', 'wali.*kelas|homeroom', 'jumlah.*siswa|student.*count'];
  ...
  test('should view class detail', async ({ page }) => {
    ...
    if (await heading.isVisible({ timeout: 5000 }).catch(() => false)) {
      ...
      if (await firstRow.isVisible({ timeout: 3000 }).catch(() => false)) {
        ...
        const detailHeading = page.getByRole('heading', { name: new RegExp(className || 'detail', 'i') });
        ...
        if (await studentSection.isVisible({ timeout: 3000 }).catch(() => false)) {
  ...
  test('should filter classes by grade level', async ({ page }) => {
    ...
    const heading = page.getByRole('heading', { name: /kelas|class/i });
    if (await heading.isVisible({ timeout: 5000 }).catch(() => false)) {
      const filterButton = page.getByRole('button', { name: /filter|tingkat/i });
      if (await filterButton.isVisible({ timeout: 3000 }).catch(() => false)) {
        await filterButton.click();
        
        const gradeOption = page.getByRole('option').or(page.getByRole('menuitem')).first();
...
test.describe('Class Management - Create and Update', () => {
  ...
  test('should create new class', async ({ page }) => {
    ...
    if (await heading.isVisible({ timeout: 5000 }).catch(() => false)) {
      ...
      // Select grade level
      const gradeSelect = page.locator('button[role="combobox"]').filter({ hasText: /tingkat|grade/i }).first();
      ...
      const teacherSelect = page.locator('button[role="combobox"]').filter({ hasText: /wali.*kelas|homeroom/i }).first();
      ...
      const newClassRow = page.locator('table tbody tr').filter({ hasText: className });
...
test.describe('Class Management - Student Enrollment', () => {
  ...
  test('should enroll student to class', async ({ page }) => {
    ...
    if (await heading.isVisible({ timeout: 5000 }).catch(() => false)) {
      ...
      const addStudentButton = page.getByRole('button', { name: /tambah.*siswa|add.*student/i });
      if (await addStudentButton.isVisible({ timeout: 3000 }).catch(() => false)) {
        ...
        if (await studentSelect.isVisible({ timeout: 3000 }).catch(() => false)) {
          ...
          const confirmButton = page.getByRole('button', { name: /tambah|add/i });
  ...
  test('should remove student from class', async ({ page }) => {
    ...
    if (await heading.isVisible({ timeout: 5000 }).catch(() => false)) {
      ...
      const studentRow = page.locator('table tbody tr, [role="listitem"]').first();
      if (await studentRow.isVisible({ timeout: 3000 }).catch(() => false)) {
        const removeButton = studentRow.getByRole('button', { name: /hapus|remove/i });
        if (await removeButton.isVisible({ timeout: 2000 }).catch(() => false)) {
          await removeButton.click();
          
          const confirmButton = page.getByRole('button', { name: /ya|yes|hapus/i });
          await confirmButton.click();
...
test.describe('Class Management - Schedule', () => {
  ...
  test('should view class schedule', async ({ page }) => {
    ...
    if (await heading.isVisible({ timeout: 5000 }).catch(() => false)) {
      ...
      const scheduleTab = page.getByRole('tab', { name: /jadwal|schedule/i }).or(
        page.getByText(/jadwal|schedule/i)
      );
      ...
      if (await scheduleTab.isVisible({ timeout: 3000 }).catch(() => false)) {
        ...
        const scheduleContent = page.locator('table, .schedule-grid, [role="grid"]');
  ...
  test('should add schedule entry', async ({ page }) => {
    ...
    if (await heading.isVisible({ timeout: 5000 }).catch(() => false)) {
      ...
      if (await scheduleTab.isVisible({ timeout: 3000 }).catch(() => false)) {
        ...
        const addScheduleButton = page.getByRole('button', { name: /tambah.*jadwal|add.*schedule/i });
        if (await addScheduleButton.isVisible({ timeout: 3000 }).catch(() => false)) {
          await addScheduleButton.click();
          
          // Fill schedule details
          const subjectSelect = page.locator('button[role="combobox"]').filter({ hasText: /mata.*pelajaran|subject/i }).first();
          if (await subjectSelect.isVisible({ timeout: 2000 }).catch(() => false)) {
            await subjectSelect.click();
            await page.getByRole('option').first().click();
          }
          
          const teacherSelect = page.locator('button[role="combobox"]').filter({ hasText: /guru|teacher/i }).first();
          if (await teacherSelect.isVisible({ timeout: 2000 }).catch(() => false)) {
            await teacherSelect.click();
            await page.getByRole('option').first().click();
          }
