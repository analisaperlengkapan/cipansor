...
export function StudentKitabTab({ studentId }: { studentId: string }) {
  ...
  if (isLoading) {
    return <div className="p-8 text-center animate-pulse">Memuat Data Kajian Kitab...</div>;
  }

  if (!report) {
    return <div className="p-8 text-center text-gray-500">Belum ada data progres kitab kuning.</div>;
  }
    ...
    <div className="space-y-4">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Daftar kitab kuning yang dipelajari beserta progresnya</CardDescription>
        </CardHeader>
        <CardContent>
          <div className="space-y-4">
            ...
            {(!report.progresses || report.progresses.length === 0) && (
              <p className="text-gray-500 text-center py-4">Tidak ada data kajian kitab</p>
            )}
          </div>
        </CardContent>
      </Card>
