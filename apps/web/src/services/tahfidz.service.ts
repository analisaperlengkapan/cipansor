...
export interface ListTahfidzParams extends PaginationParams, DateRangeParams, UnitFilterParams {
    studentId?: string;
...
export const tahfidzService = {
     ...
     */
    async list(params?: ListTahfidzParams): Promise<PaginatedResponse<TahfidzRecord>> {
        const response = await api.get<PaginatedResponse<TahfidzRecord>>('/tahfidz', {
            params
        });
    ...
    async create(input: CreateTahfidzInput): Promise<TahfidzRecord> {
        const response = await api.post<ApiResponse<TahfidzRecord>>('/tahfidz', input);
        return response.data.data;
    },
     ...
     */
    async getDashboardStats(params?: UnitFilterParams): Promise<TahfidzDashboardStats> {
     ...
     */
    async getLeaderboard(params?: UnitFilterParams & { limit?: number }): Promise<{
        students: Array<{
            rank: number;
            studentId: string;
            studentName: string;
     ...
     */
    async generateCertificate(studentId: string, options?: {
        type: 'juz' | 'surah' | 'full';
        juzNumber?: number;
        surahNumber?: number;
    }): Promise<{ downloadUrl: string; certificateId: string }> {
        const response = await api.post<ApiResponse<{ downloadUrl: string; certificateId: string }>>(
            `/tahfidz/students/${studentId}/certificate`,
            options
        );
        return response.data.data;
    }
