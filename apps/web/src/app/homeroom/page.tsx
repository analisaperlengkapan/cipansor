...
export default function HomeroomDashboardPage() {
  ...
  const { class: classInfo, dashboardSummary: summary, attendanceSummary } = dashboardData;
  ...
  const totalAttendanceRecords = attendanceSummary.reduce((acc, curr) => acc + curr.count, 0);
  const getCount = (status: string) => attendanceSummary.find(s => s.status === status)?.count || 0;
  ...
  return (
    <div className="container mx-auto py-6 space-y-6">
      ...
      <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-5">
        ...
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm font-medium text-muted-foreground">Kehadiran (Bulan Ini)</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="flex items-center gap-2">
              ...
              <CardContent>
                <div className="grid grid-cols-5 gap-3 text-center">
                  ...
                  <div className="p-3 rounded-lg bg-blue-100 dark:bg-blue-900/20">
                    ...
                    <div className="text-2xl font-bold text-blue-600">{sickCount}</div>
                    <p className="text-xs text-muted-foreground">Sakit</p>
                  </div>
                  <div className="p-3 rounded-lg bg-purple-100 dark:bg-purple-900/20">
                    <UserX className="h-6 w-6 text-purple-600 mx-auto mb-1" />
                    <div className="text-2xl font-bold text-purple-600">{permitCount}</div>
                    ...
                    {summary.recentAchievements.map((achievement) => (
                      <div key={achievement.id} className="p-3 rounded-lg border border-green-200 bg-green-50 dark:border-green-800 dark:bg-green-900/20">
                        <div className="flex justify-between items-start">
                          <div>
                            <p className="font-medium text-green-800 dark:text-green-200">{achievement.student.user.name}</p>
                          ...
                          {achievement.points !== undefined && (
                            <Badge className="bg-green-600">
                                {achievement.type === 'TAHFIDZ' ? 'Lulus' : `+${achievement.points} poin`}
                            </Badge>
                          )}
                        </div>
                      </div>
                    ...
                    {summary.recentViolations.map((violation) => {
                      ...
                      return (
                        <div key={violation.id} className="p-3 rounded-lg border border-red-200 bg-red-50 dark:border-red-800 dark:bg-red-900/20">
                          <div className="flex justify-between items-start">
                            <div>
                              <p className="font-medium text-red-800 dark:text-red-200">{violation.student.user.name}</p>
                              <p className="text-sm text-muted-foreground">{violation.description}</p>
                              <p className="text-xs text-muted-foreground mt-1">
                                {violation.category} • {new Date(violation.occurredAt).toLocaleDateString('id-ID')}
                              </p>
                            </div>
                      ...
                      <tr key={student.id} className="border-b hover:bg-muted/50">
                        ...
                        <td className="py-3 px-2 text-center">
                          <div className="flex items-center justify-center gap-1">
                            <Link href={`/homeroom/${classInfo.id}/students/${student.id}`}>
