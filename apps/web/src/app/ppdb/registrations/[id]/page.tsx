...
export default function RegistrationDetailPage() {
  ...
  const handleStatusChange = async (status: RegistrationStatus) => {
    try {
      ...
      toast.success(`Status berhasil diubah ke ${REGISTRATION_STATUS_LABELS[status]}`);
    } catch (error) {
  ...
  const handleScheduleTest = async () => {
    try {
      await scheduleTest.mutateAsync({ id, testDate: new Date(testDate).toISOString(), notes });
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="flex items-start gap-4">
            ...
            <div>
              ...
              <p className="text-muted-foreground flex items-center gap-2">
                ...
                {format(new Date(registration.createdAt), 'd MMM yyyy HH:mm', { locale: idLocale })}
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-2">
            ...
            {registration.status === 'SUBMITTED' && (
              <Button onClick={() => handleStatusChange('DOCUMENT_REVIEW')} className="bg-blue-600 hover:bg-blue-700">
            ...
            {registration.status === 'TEST_SCHEDULED' && (
              <Button onClick={() => setScoreDialogOpen(true)} className="bg-purple-600 hover:bg-purple-700">
                <GraduationCap className="h-4 w-4 mr-2" />
                Input Nilai Tes
              </Button>
            )}

            {(registration.status === 'TEST_COMPLETED' || registration.status === 'INTERVIEW_COMPLETED') && (
          ...
          <TabsContent value="details">
            <Card>
              ...
              <CardContent className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-6">
                ...
                <div className="space-y-1">
                  <Label className="text-muted-foreground text-xs uppercase tracking-wider">Nama Panggilan</Label>
                  <p className="font-medium">{registration.nickname || '-'}</p>
                </div>
                <div className="space-y-1">
                  <Label className="text-muted-foreground text-xs uppercase tracking-wider">Jenis Kelamin</Label>
                  <p className="font-medium">{registration.gender === 'MALE' ? 'Laki-laki' : 'Perempuan'}</p>
                </div>
                <div className="space-y-1">
                  <Label className="text-muted-foreground text-xs uppercase tracking-wider">Tempat, Tanggal Lahir</Label>
                  <p className="font-medium">
                    {registration.birthPlace}, {format(new Date(registration.birthDate), 'd MMMM yyyy', { locale: idLocale })}
                  </p>
                </div>
                <div className="space-y-1">
                  <Label className="text-muted-foreground text-xs uppercase tracking-wider">NIK</Label>
                  <p className="font-medium font-mono">{registration.nationalId || '-'}</p>
                </div>
                <div className="space-y-1">
                  <Label className="text-muted-foreground text-xs uppercase tracking-wider">Asal Sekolah</Label>
                ...
                <div className="md:col-span-2 space-y-1">
                  ...
                  <p className="text-muted-foreground text-sm">
                    {registration.village}, {registration.district}, {registration.city}, {registration.province} {registration.postalCode}
                  </p>
                </div>
              </CardContent>
            </Card>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <Card>
                ...
                <CardContent className="space-y-4">
                  ...
                  <div className="space-y-1">
                    <Label className="text-muted-foreground text-xs uppercase tracking-wider">Pekerjaan</Label>
                    <p className="font-medium">{registration.fatherOccupation || '-'}</p>
                  </div>
                  <div className="space-y-1">
                    <Label className="text-muted-foreground text-xs uppercase tracking-wider">No. HP</Label>
              ...
              <CardContent>
                <div className="space-y-4">
                  {[
                    ...
                    { label: 'Akte Kelahiran', url: registration.birthCertificateUrl },
                    { label: 'Kartu Keluarga', url: registration.familyCardUrl },
                    { label: 'Ijazah / SKL', url: registration.diplomaUrl },
                    { label: 'Surat Keterangan Sehat', url: registration.healthCertificateUrl },
                  ].map((doc, idx) => (
                    <div key={idx} className="flex items-center justify-between p-4 border rounded-lg hover:bg-slate-50 transition-colors">
                      <div className="flex items-center gap-3">
                        <FileText className={`h-5 w-5 ${doc.url ? 'text-blue-600' : 'text-gray-400'}`} />
                      ...
                      {doc.url ? (
                        ...
                        <Badge variant="outline" className="text-gray-500">Belum diupload</Badge>
                      )}
                    </div>
                  ))}
                </div>
                ...
                <div className="space-y-6 border-l-2 border-slate-200 pl-6 ml-2">
                  <div className="relative">
                    ...
                    <p className="text-xs text-muted-foreground">
                      {format(new Date(registration.createdAt), 'd MMMM yyyy HH:mm', { locale: idLocale })}
                    </p>
                  </div>

                  {registration.testDate && (
                    <div className="relative">
                      <div className={`absolute -left-[31px] h-4 w-4 rounded-full border-4 border-white ${registration.testScore ? 'bg-green-500' : 'bg-blue-500'}`} />
                      <h4 className="font-semibold text-sm">Jadwal Tes</h4>
                      <p className="text-sm">
                        Dilaksanakan pada {format(new Date(registration.testDate), 'd MMMM yyyy', { locale: idLocale })}
                      </p>
                      {registration.testScore !== undefined && (
                         <div className="mt-2 p-3 bg-slate-50 rounded border">
                           <p className="font-semibold text-sm">Nilai Tes: {registration.testScore}</p>
                           {registration.testNotes && <p className="text-xs text-muted-foreground">Catatan: {registration.testNotes}</p>}
                         </div>
                      )}
                    </div>
                  )}
                  ...
                  {registration.acceptedAt && (
                    <div className="relative">
                      ...
                      <h4 className="font-semibold text-sm text-green-700">Diterima</h4>
                      <p className="text-xs text-muted-foreground">
                        {format(new Date(registration.acceptedAt), 'd MMMM yyyy', { locale: idLocale })}
                      </p>
                    </div>
                  )}
                  ...
                  {registration.rejectedAt && (
                    <div className="relative">
                      ...
                      <h4 className="font-semibold text-sm text-red-700">Ditolak</h4>
                      <p className="text-xs text-muted-foreground">
                        {format(new Date(registration.rejectedAt), 'd MMMM yyyy', { locale: idLocale })}
                      </p>
          ...
          <DialogContent>
            ...
            <div className="space-y-4 py-2">
              ...
              <div className="space-y-2">
                ...
                <Textarea value={notes} onChange={(e) => setNotes(e.target.value)} placeholder="Lokasi tes, perlengkapan, dll..." />
              </div>
            </div>
            <DialogFooter>
              <Button variant="outline" onClick={() => setTestDialogOpen(false)}>Batal</Button>
              <Button onClick={handleScheduleTest} disabled={!testDate}>Simpan Selesai</Button>
            </DialogFooter>
          </DialogContent>
        </Dialog>
          ...
          <DialogContent>
            ...
            <div className="space-y-4 py-2">
              <div className="space-y-2">
                ...
                <Input type="number" min="0" max="100" value={testScore} onChange={(e) => setTestScore(e.target.value)} />
              </div>
              <div className="space-y-2">
                <Label>Catatan</Label>
                <Textarea value={notes} onChange={(e) => setNotes(e.target.value)} placeholder="Catatan hasil tes..." />
              </div>
            </div>
            <DialogFooter>
              <Button variant="outline" onClick={() => setScoreDialogOpen(false)}>Batal</Button>
              <Button onClick={handleRecordScore} disabled={!testScore}>Simpan</Button>
            </DialogFooter>
          </DialogContent>
        </Dialog>
        ...
        <AlertDialog open={acceptDialogOpen} onOpenChange={setAcceptDialogOpen}>
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                Tindakan ini akan mengubah status menjadi DITERIMA. Anda akan dapat membuat tagihan masuk setelah ini.
              </AlertDialogDescription>
            </AlertDialogHeader>
            <div className="py-2">
              <Label className="mb-2 block">Catatan Penerimaan (Opsional)</Label>
              <Textarea value={notes} onChange={(e) => setNotes(e.target.value)} placeholder="Selamat bergabung..." />
            </div>
            <AlertDialogFooter>
              <AlertDialogCancel>Batal</AlertDialogCancel>
              <AlertDialogAction onClick={handleAccept} className="bg-green-600 hover:bg-green-700">Terima Santri</AlertDialogAction>
            </AlertDialogFooter>
          </AlertDialogContent>
        </AlertDialog>
        ...
        <AlertDialog open={rejectDialogOpen} onOpenChange={setRejectDialogOpen}>
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                Tindakan ini tidak dapat dibatalkan. Mohon sertakan alasan penolakan.
              </AlertDialogDescription>
            </AlertDialogHeader>
            <div className="py-2">
              <Label className="mb-2 block">Alasan Penolakan *</Label>
              <Textarea value={notes} onChange={(e) => setNotes(e.target.value)} placeholder="Mohon maaf..." />
            </div>
            <AlertDialogFooter>
              <AlertDialogCancel>Batal</AlertDialogCancel>
              <AlertDialogAction onClick={handleReject} disabled={!notes} className="bg-red-600 hover:bg-red-700">Tolak</AlertDialogAction>
            </AlertDialogFooter>
          </AlertDialogContent>
        </AlertDialog>
