...
test.describe('Extracurricular - Features', () => {
  test('should display activity or club list', async ({ page }) => {
    ...
    const hasActivities = await page.locator('table, [class*="activity"], [class*="ekskul"]')
      .first()
      .isVisible({ timeout: 5000 }).catch(() => false);
    
    expect(hasActivities || page.url().includes('extracurricular')).toBeTruthy();
  });
  ...
  test('should display student participation', async ({ page }) => {
    ...
    const content = await page.content();
    const hasParticipation = content.includes('Peserta') || 
                            content.includes('Siswa') || 
                            content.includes('Member');
    
    expect(hasParticipation || page.url().includes('extracurricular')).toBeTruthy();
  });
  ...
  test('should show activity schedules', async ({ page }) => {
    ...
    const content = await page.content();
    const hasSchedule = content.includes('Jadwal') || 
