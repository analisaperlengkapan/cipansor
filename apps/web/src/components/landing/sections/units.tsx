...
export function UnitsSection({ locale }: { locale: Locale }) {
  const copy = homeContentFor(locale).units;
            ...
            >
              <CardHeader>
                ...
                <p className="text-sm font-medium text-primary">
                  {unitText[unit.slug]?.tagline ?? unit.tagline}
                </p>
              </CardHeader>
              <CardContent className="flex flex-1 flex-col">
                <CardDescription className="flex-1 text-base leading-relaxed">
                  {unitText[unit.slug]?.description ?? unit.description}
                </CardDescription>
