...
const fetchSchedule = async (weekStart: Date, halaqohId?: string): Promise<MurojaahScheduleItem[]> => {
        ...
        <div className="flex justify-between items-start">
          <div className="space-y-1">
            ...
            <div className="flex items-center gap-2 text-xs text-muted-foreground">
              ...
              <span>Juz {item.juzStart}{item.juzEnd !== item.juzStart ? `-${item.juzEnd}` : ''}</span>
            </div>
            {item.scheduledTime && (
              <div className="flex items-center gap-2 text-xs text-muted-foreground">
          ...
          <div className="flex flex-col items-end gap-1">
            <Badge variant="outline" className={statusColors[item.status]}>
              {item.status === 'SCHEDULED' ? 'Dijadwalkan' : item.status === 'COMPLETED' ? 'Selesai' : 'Terlewat'}
            </Badge>
...
function DayColumn({ daySchedule }: { daySchedule: DaySchedule }) {
  ...
  return (
    <div className={`flex-1 min-w-[150px] ${isToday ? 'bg-primary/5 rounded-lg' : ''}`}>
      <div className={`p-2 text-center border-b ${isToday ? 'bg-primary/10' : 'bg-muted/50'}`}>
...
export default function MurojaahSchedulePage() {
  ...
  const { data: scheduleItems, isLoading } = useQuery({
    queryKey: ['murojaah-schedule', currentWeekStart.toISOString(), selectedHalaqoh],
    queryFn: () => fetchSchedule(currentWeekStart, selectedHalaqoh !== 'all' ? selectedHalaqoh : undefined),
  });
            ...
            <div className="flex items-center gap-2">
              ...
              <div className="flex items-center gap-2 px-4 py-2 bg-muted rounded-md">
                ...
                <span className="font-medium">
                  ...
                  {format(addDays(currentWeekStart, 6), 'd MMM yyyy', { locale: localeId })}
                </span>
              </div>
          ...
          {isLoading ? (
            ...
            <div className="flex gap-2 overflow-x-auto pb-4">
              {weekDays.map((daySchedule) => (
                <DayColumn key={daySchedule.date.toISOString()} daySchedule={daySchedule} />
              ))}
            </div>
          )}
        </CardContent>
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <div className="text-2xl font-bold">
                  {scheduleItems?.filter((i) => i.status === 'SCHEDULED').length || 0}
                </div>
        ...
        <Card>
          <CardContent className="p-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <div className="text-2xl font-bold">
                  {scheduleItems?.filter((i) => i.status === 'COMPLETED').length || 0}
                </div>
        ...
        <Card>
          <CardContent className="p-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <div className="text-2xl font-bold">
                  {scheduleItems?.filter((i) => i.status === 'MISSED').length || 0}
                </div>
