    ...
    <MainLayout>
      <div className="space-y-6">
        ...
        {viewMode === 'calendar' ? (
          ...
          <Card>
            <CardHeader>
              <CardTitle>Daftar Event Bulan {format(currentDate, 'MMMM yyyy', { locale: idLocale })}</CardTitle>
              <CardDescription>
            ...
            <CardContent>
              {isLoading ? (
                ...
                <div className="space-y-3">
                  {filteredEvents.sort((a, b) => new Date(a.startDate).getTime() - new Date(b.startDate).getTime()).map((event) => {
                    const catConfig = getEventCategoryConfig(event.category);
                    const Icon = iconMap[catConfig?.icon || 'Calendar'] || CalendarIcon;
            ...
            <CardContent>
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                {EVENT_CATEGORIES.slice(0, 4).map((cat) => {
                  const count = filteredEvents.filter(e => e.category === cat.value).length;
                  ...
                  return (
                    ...
                    >
                      <div className="h-8 w-8 mx-auto mb-2" style={{ color: cat.color }}>
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-muted-foreground">
                          {format(new Date(event.startDate), 'd MMM yyyy', { locale: idLocale })}
                        </p>
                      </div>
                    </div>
                  );
                  ...
                  <div className="flex items-start gap-3">
                    <div 
                      ...
                      style={{ 
                        ...
                        color: getEventCategoryConfig(selectedEvent.category)?.color 
                      }}
                    >
                      {(() => {
                        const catConfig = getEventCategoryConfig(selectedEvent.category);
                        const Icon = iconMap[catConfig?.icon || 'Calendar'] || CalendarIcon;
                    ...
                    <div className="flex-1">
                      ...
                      <Badge className={getEventCategoryConfig(selectedEvent.category)?.bgColor}>
                ...
                <div className="space-y-4 pt-4">
                  ...
                  <div className="space-y-2">
                    <div className="flex items-center gap-2 text-sm">
                      ...
                      <span>
                        {format(new Date(selectedEvent.startDate), 'd MMMM yyyy', { locale: idLocale })}
                        {selectedEvent.startDate !== selectedEvent.endDate && (
                          <> - {format(new Date(selectedEvent.endDate), 'd MMMM yyyy', { locale: idLocale })}</>
                        )}
                      </span>
                    </div>
