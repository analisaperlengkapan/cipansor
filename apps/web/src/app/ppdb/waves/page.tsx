...
export default function PPDBWavesPage() {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      const errorMessage = error instanceof Error ? error.message : 'Gagal menghapus gelombang';
  ...
  const handleStatusChange = async (id: string, status: WaveStatus) => {
    try {
      ...
      toast.success(`Status gelombang berhasil diubah ke ${WAVE_STATUSES.find(s => s.value === status)?.label}`);
    } catch (error: unknown) {
      ...
      <div className="grid gap-4 md:grid-cols-4 mb-6">
        <Card>
          <CardContent className="p-4">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm text-muted-foreground">Total Gelombang</span>
            </div>
        ...
        <Card>
          <CardContent className="p-4">
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm text-muted-foreground">Gelombang Aktif</span>
            </div>
            <p className="text-2xl font-bold text-green-600">{stats?.openWaves || 0}</p>
          </CardContent>
        </Card>
        <Card>
          <CardContent className="p-4">
        ...
        <Card>
          <CardContent className="p-4">
            ...
            <p className="text-2xl font-bold text-emerald-600">{stats?.acceptedRegistrants || 0}</p>
          </CardContent>
        </Card>
      </div>
              ...
              {isLoading ? (
                ...
                <TableRow>
                  <TableCell colSpan={7} className="text-center py-8">
                    ...
                    <p className="text-muted-foreground">Tidak ada gelombang PPDB</p>
                ...
                waves.map((wave) => {
                  ...
                  return (
                    <TableRow key={wave.id}>
                      <TableCell>
                        <div>
                          ...
                          <p className="text-sm text-muted-foreground">
                            {format(new Date(wave.startDate), 'd MMM', { locale: localeId })} -{' '}
                      ...
                      <TableCell>{formatRegistrationFee(wave.registrationFee)}</TableCell>
                        ...
                        <div className="flex items-center gap-1">
                          ...
                          <DropdownMenu>
                            ...
                            <DropdownMenuContent align="end">
                              ...
                              {wave.status === 'DRAFT' && (
                                <DropdownMenuItem onClick={() => handleStatusChange(wave.id, 'OPEN')}>
                                  <Play className="h-4 w-4 mr-2" />
                                  Buka Pendaftaran
                                </DropdownMenuItem>
                              )}
                              {wave.status === 'OPEN' && (
                                <DropdownMenuItem onClick={() => handleStatusChange(wave.id, 'CLOSED')}>
                                  <Pause className="h-4 w-4 mr-2" />
                                  Tutup Pendaftaran
                                </DropdownMenuItem>
                              )}
                              {wave.status === 'CLOSED' && (
                                <DropdownMenuItem onClick={() => handleStatusChange(wave.id, 'COMPLETED')}>
