...
export default function LetterDetailPage({ params }: { params: { id: string } }) {
  ...
  const handleReview = async (action: 'APPROVE' | 'REJECT') => {
    try {
      ...
      toast.success(`Surat berhasil ${action === 'APPROVE' ? 'disetujui' : 'ditolak'}`);
    } catch (error) {
  ...
  const handleCreateDisposition = async () => {
    ...
    try {
      ...
      setDispositionData({ recipientId: '', instruction: '', deadline: '', notes: '' });
    } catch (error) {
        ...
        <DialogContent className="sm:max-w-[500px]">
          ...
          <div className="grid gap-4 py-4">
            <div className="grid gap-2">
              ...
              <Select
                onValueChange={(val) => setDispositionData({...dispositionData, recipientId: val})}
            ...
            <div className="grid gap-2">
              ...
              <Textarea
                ...
                onChange={(e) => setDispositionData({...dispositionData, instruction: e.target.value})}
              />
            </div>
            <div className="grid gap-2">
          ...
          <div className="flex justify-end gap-3">
            <Button variant="outline" onClick={() => setDispositionOpen(false)}>Batal</Button>
      ...
      <div className="grid gap-6 md:grid-cols-3">
            ...
            <CardContent className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  <p>{letter.recipientName || letter.recipientInstance || '-'}</p>
                </div>
                <div>
                  <label className="text-xs font-medium text-muted-foreground">Sifat</label>
                  <p>{letter.nature}</p>
                </div>
                <div>
              ...
              <div>
                <label className="text-xs font-medium text-muted-foreground">Isi Ringkas</label>
              ...
              {letter.fileUrl && (
                <div className="flex items-center gap-2 p-3 border rounded-md">
                  ...
                  <span className="text-sm font-medium flex-1">Lampiran Surat.pdf</span>
        ...
        <div className="space-y-6">
          ...
          <Card>
            ...
            <CardContent className="space-y-3">
              <Button className="w-full" variant="outline" onClick={() => setDispositionOpen(true)}>
              ...
              <div className="pt-4 border-t space-y-3">
                <p className="text-xs font-medium text-muted-foreground mb-2">Persetujuan</p>
                <Textarea
            ...
            <CardContent>
              <div className="space-y-4">
                {letter.reviewers?.map((reviewer, index) => (
                  <div key={reviewer.id} className="flex items-start gap-3 text-sm">
                    <div className={`
                    ...
                    <div>
                      ...
                      {reviewer.notes && (
                        <p className="text-xs mt-1 bg-muted p-2 rounded">"{reviewer.notes}"</p>
                      )}
                    </div>
                  </div>
                ))}
                {(!letter.reviewers || letter.reviewers.length === 0) && (
                  <p className="text-sm text-muted-foreground text-center">Tidak ada reviewer assigned.</p>
                )}
              </div>
            </CardContent>
          </Card>
