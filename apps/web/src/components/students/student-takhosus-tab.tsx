        ...
        <CardContent>
          <div className="space-y-4">
            {enrollmentList.map((enr: any) => (
              <div key={enr.id} className="flex justify-between items-center border-b pb-4 last:border-0 last:pb-0">
                <div>
                  <h4 className="font-medium text-lg text-gray-900">{enr.halaqoh?.name}</h4>
                  <p className="text-sm text-gray-500">Program: {enr.halaqoh?.program}</p>
                  <p className="text-xs text-gray-400 mt-1">Bergabung: {format(new Date(enr.joinedAt), "dd MMM yyyy")}</p>
                </div>
                <div className="text-right">
                  <div className="text-lg font-bold text-blue-600">{enr.progressPercentage || 0}%</div>
            ...
            {enrollmentList.length === 0 && (
              <p className="text-gray-500 text-center py-4">Tidak mengikuti program takhosus apapun.</p>
            )}
          </div>
        </CardContent>
      </Card>
        ...
        <CardContent>
          <div className="space-y-4">
            {sanadList.map((sanad: any) => (
              <div key={sanad.id} className="flex justify-between items-center border-b pb-4 last:border-0 last:pb-0">
                <div>
                  <h4 className="font-medium text-gray-900">Juz {sanad.juz}</h4>
                  <p className="text-sm text-gray-500">Penguji: {sanad.teacher?.name}</p>
                  <p className="text-xs text-gray-400">{format(new Date(sanad.certifiedAt || sanad.createdAt), "dd MMM yyyy")}</p>
                </div>
                <div className="text-right">
            ...
            {sanadList.length === 0 && (
              <p className="text-gray-500 text-center py-4">Belum ada riwayat sertifikasi sanad.</p>
            )}
          </div>
        </CardContent>
      </Card>
