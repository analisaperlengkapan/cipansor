...
test.describe("Integrated Performance Management (/kinerja) E2E Flows", () => {
  ...
  test("main performance hub renders key navigation sections", async ({
    ...
    await expect(page.locator("h1")).toContainText(
      /Perencanaan Strategis hingga Perjanjian & Evaluasi Kinerja/i,
    );
    await expect(
      page.getByText("Perjanjian Kinerja (PK)", { exact: true }),
    ).toBeVisible();
    await expect(
      page.getByText("Evaluasi & Perilaku SAFTI", { exact: true }),
    ).toBeVisible();
  });
  ...
  test("periodic evaluation hub loads monthly evaluations", async ({
    ...
    await expect(page.locator("h1")).toContainText(
      /Evaluasi Kinerja Periodik/i,
    );
    await expect(
      page.getByRole("button", { name: "Mulai Evaluasi Bulanan" }),
    ).toBeVisible();
  });
  ...
  test("interactive PK creation modal opens and validates form inputs", async ({
    ...
    await page.goto("/kinerja/pk");
    await page
      .getByRole("button", { name: "Buat Perjanjian Kinerja", exact: true })
      .click();
    await expect(
  ...
  test("end-to-end flow: submit PK, submit evaluation creation, and verify analytics drilldown", async ({
    page,
  }) => {
    ...
    try {
      ...
      await page.goto("/kinerja/evaluasi");
      await expect(page.locator("h1")).toContainText(/Evaluasi/i);

      // Open evaluation dialog
      await page.click("button:has-text('Mulai Evaluasi Bulanan')");
      await expect(
      ...
      await page.goto("/kinerja/analytics");
      await expect(page.locator("h1")).toContainText(
        /Analytics & Strategy Map/i,
      );
      await expect(
        page.locator("text=Drilldown Capaian per Unit Kerja"),
      ).toBeVisible();
      await expect(
  ...
  test("periodic evaluation hub loads real page structure", async ({
    page,
  }) => {
    await page.goto("/kinerja/evaluasi");
    await expect(page.locator("h1")).toContainText(
