    ...
    queryFn: async (): Promise<TeacherStats> => {
      try {
        ...
        return {
          ...
          todayScheduleCount: todaySchedules.length || scheduleData?.data?.length || 0,
  ...
  return useQuery({
    ...
    queryFn: async (): Promise<TeachingScheduleItem[]> => {
      try {
        ...
        }).sort((a: TeachingScheduleItem, b: TeachingScheduleItem) => a.time.localeCompare(b.time));
      } catch (error) {
...
export function useTeacherDashboard() {
  ...
  return {
    ...
    isLoading: stats.isLoading || todaySchedule.isLoading || recentSetoran.isLoading,
...
function getDefaultTeacherSchedule(): TeachingScheduleItem[] {
  ...
  const defaultSchedule = [
    { time: '07:00', activity: 'Tahfidz Pagi - Kelas 7A', type: 'TAHFIDZ' as const },
    { time: '09:00', activity: 'Tahfidz - Kelas 8B', type: 'TAHFIDZ' as const },
    { time: '10:30', activity: 'Setoran Hafalan - Kelas 9A', type: 'TAHFIDZ' as const },
    { time: '13:00', activity: "Muraja'ah - Kelas 7B", type: 'TAHFIDZ' as const },
...
export function getGradeDisplay(grade: string): { label: string; color: string } {
  const grades: Record<string, { label: string; color: string }> = {
    ...
    'JAYYID_JIDDAN': { label: 'Jayyid Jiddan', color: 'bg-blue-100 text-blue-800' },
...
export function getScheduleStatusDisplay(status: string): { label: string; color: string } {
  const statuses: Record<string, { label: string; color: string }> = {
