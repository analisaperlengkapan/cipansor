...
describe("Permit Gate Logic", () => {
  ...
  describe("markDeparted", () => {
    ...
    it("should throw if permit is not APPROVED", async () => {
      prismaMock.permit.findUnique.mockResolvedValue({ ...mockPermit, status: PermitStatus.PENDING } as any);
  ...
  describe("markReturned", () => {
    it("should mark permit as completed and returned", async () => {
      prismaMock.permit.findUnique.mockResolvedValue(mockPermit as any);
      prismaMock.permit.update.mockResolvedValue({ ...mockPermit, status: PermitStatus.COMPLETED, returnedAt: new Date() } as any);
  ...
  describe("createPermit", () => {
    it("should generate a unique code", async () => {
      ...
      expect(prismaMock.permit.create).toHaveBeenCalledWith(expect.objectContaining({
