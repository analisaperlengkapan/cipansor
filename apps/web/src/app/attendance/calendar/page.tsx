...
export default function AttendanceCalendarPage() {
  ...
  const { data: calendarData, isLoading } = useAttendanceCalendar(classId, year, month);
  ...
  const getAttendanceColor = (day: { present: number; absent: number; total: number } | undefined) => {
    ...
    if (rate >= 75) return 'bg-yellow-100 dark:bg-yellow-900/30 border-yellow-300';
    if (rate >= 50) return 'bg-orange-100 dark:bg-orange-900/30 border-orange-300';
    ...
    <MainLayout>
        ...
        <CardContent>
          <div className="flex flex-wrap gap-4">
            ...
            <Select value={classId} onValueChange={setClassId}>
              ...
              <SelectContent>
                {classes.length === 0 ? (
                  <SelectItem value="" disabled>Pilih unit terlebih dahulu</SelectItem>
                ) : (
                  classes.map((cls) => (
      ...
      {!classId ? (
        <Card>
          <CardContent className="flex flex-col items-center justify-center py-16">
            ...
            <p className="text-lg font-medium text-muted-foreground">Pilih kelas untuk melihat kalender kehadiran</p>
          </CardContent>
        </Card>
      ) : (
        <>
            ...
            <div className="grid gap-4 md:grid-cols-3 mb-6">
              <Card>
                <CardContent className="p-4">
                  ...
                  <p className="text-2xl font-bold">{calendarData.summary.totalStudents}</p>
                </CardContent>
              </Card>
              <Card>
                <CardContent className="p-4">
                  <div className="flex items-center gap-2 mb-2">
                    ...
                    <span className="text-sm text-muted-foreground">Hari Aktif</span>
                  </div>
                  <p className="text-2xl font-bold">{calendarData.summary.totalSchoolDays}</p>
                </CardContent>
              </Card>
              <Card>
                <CardContent className="p-4">
                  ...
                  <p className="text-2xl font-bold text-green-600">{calendarData.summary.avgAttendanceRate}%</p>
                </CardContent>
              </Card>
            </div>
          )}
                ...
                <>
                  ...
                  <div className="grid grid-cols-7 gap-2">
                    <TooltipProvider>
                      {calendarDays.map((day, idx) => {
                        ...
                        const rate = hasData ? Math.round((attendance.present / attendance.total) * 100) : 0;
                        ...
                        return (
                          <Tooltip key={dateKey}>
                            <TooltipTrigger asChild>
                              <button
                                onClick={() => hasData ? setSelectedDay(dateKey) : null}
                                className={cn(
                                  'h-20 rounded-lg border p-2 text-left transition-all',
                                  hasData ? 'cursor-pointer hover:shadow-md' : 'cursor-default',
                                  isToday(day) && 'ring-2 ring-primary',
                                  isWeekend && !hasData && 'bg-gray-50 dark:bg-gray-900',
                                  ...
                                  {hasData && (
                                    <div className="flex-1 flex flex-col justify-end">
                                      <span className={cn(
                                        ...
                                        rate >= 75 && rate < 90 && 'text-yellow-600',
                                        rate >= 50 && rate < 75 && 'text-orange-600',
                            ...
                            {hasData && (
                              <TooltipContent>
                                <div className="text-sm">
                                  <p className="font-medium">{format(day, 'd MMMM yyyy', { locale: localeId })}</p>
                  ...
                  <div className="mt-4 flex flex-wrap gap-4 justify-center">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-xs text-muted-foreground">≥90% Hadir</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <div className="w-4 h-4 rounded bg-yellow-100 border border-yellow-300" />
                      <span className="text-xs text-muted-foreground">75-89% Hadir</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <div className="w-4 h-4 rounded bg-orange-100 border border-orange-300" />
                      <span className="text-xs text-muted-foreground">50-74% Hadir</span>
                    </div>
                    <div className="flex items-center gap-2">
                      <div className="w-4 h-4 rounded bg-red-100 border border-red-300" />
                      <span className="text-xs text-muted-foreground">&lt;50% Hadir</span>
                    </div>
                    <div className="flex items-center gap-2">
      ...
      <Dialog open={!!selectedDay} onOpenChange={(open) => !open && setSelectedDay(null)}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>
              Detail Kehadiran - {selectedDay && format(new Date(selectedDay), 'd MMMM yyyy', { locale: localeId })}
            </DialogTitle>
            <DialogDescription>
            ...
            <div className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div className="p-4 rounded-lg bg-green-50 dark:bg-green-900/20">
                  ...
                  <p className="text-2xl font-bold text-green-600">{selectedDayData.present}</p>
                </div>
                <div className="p-4 rounded-lg bg-red-50 dark:bg-red-900/20">
                  <p className="text-sm text-muted-foreground">Tidak Hadir</p>
                  <p className="text-2xl font-bold text-red-600">{selectedDayData.absent}</p>
                </div>
                <div className="p-4 rounded-lg bg-yellow-50 dark:bg-yellow-900/20">
                  <p className="text-sm text-muted-foreground">Terlambat</p>
                  <p className="text-2xl font-bold text-yellow-600">{selectedDayData.late}</p>
                </div>
                <div className="p-4 rounded-lg bg-blue-50 dark:bg-blue-900/20">
                  <p className="text-sm text-muted-foreground">Sakit</p>
                  <p className="text-2xl font-bold text-blue-600">{selectedDayData.sick}</p>
                </div>
              ...
              <div className="space-y-2">
                <div className="flex justify-between text-sm">
                  ...
                  <span className="font-medium">
                    {Math.round((selectedDayData.present / selectedDayData.total) * 100)}%
                  </span>
                </div>
                <Progress 
                  value={(selectedDayData.present / selectedDayData.total) * 100}
                  className="h-2"
                />
              </div>
              ...
              <div className="flex justify-end gap-2">
                ...
                <Button asChild>
                  <Link href={`/attendance?classId=${classId}&date=${selectedDay}`}>
                    Lihat Detail
                  </Link>
                </Button>
              </div>
