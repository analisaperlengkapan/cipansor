          ...
          <div key={component.key} className="space-y-3">
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                ...
                <Badge className={getGradeColor(gradeInfo.grade)}>{gradeInfo.grade}</Badge>
              </div>
            </div>
              ...
              <div className={`grid gap-4 ${hasPartner ? 'md:grid-cols-2' : ''}`}>
                ...
                <div className="flex items-center gap-3 p-3 border rounded-lg">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-muted-foreground">{muhadatsah.student?.nis}</p>
                  </div>
                ...
                {hasPartner && (
                  <div className="flex items-center gap-3 p-3 border rounded-lg">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-sm text-muted-foreground">{muhadatsah.partner?.nis}</p>
                    </div>
              ...
              {hasPartner ? (
                <Tabs value={activeTab} onValueChange={setActiveTab}>
                  <TabsList className="grid w-full grid-cols-2">
                    <TabsTrigger value="student1" className="flex items-center gap-2">
                      <User className="h-4 w-4" />
                      {muhadatsah.student?.name?.split(' ')[0]}
                      <Badge variant="secondary" className="ml-1">{student1Average}</Badge>
                    </TabsTrigger>
                    <TabsTrigger value="student2" className="flex items-center gap-2">
                      <User className="h-4 w-4" />
                      {muhadatsah.partner?.name?.split(' ')[0]}
                      <Badge variant="secondary" className="ml-1">{student2Average}</Badge>
                    </TabsTrigger>
                  </TabsList>
                  <div className="mt-6">
                    <TabsContent value="student1">
                      {renderScoringSliders(student1Scores, setStudent1Scores, muhadatsah.student?.name || 'Santri 1')}
                    </TabsContent>
                    <TabsContent value="student2">
                      {renderScoringSliders(student2Scores, setStudent2Scores, muhadatsah.partner?.name || 'Partner')}
                    </TabsContent>
                  </div>
                </Tabs>
              ) : (
                renderScoringSliders(student1Scores, setStudent1Scores, muhadatsah.student?.name || 'Santri')
              )}
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent className="space-y-3">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm">{getLanguageLabel(muhadatsah.language)}</span>
              </div>
              <div className="flex items-center gap-2">
                <Clock className="h-4 w-4 text-muted-foreground" />
                <span className="text-sm">
                  {format(new Date(muhadatsah.scheduledAt), 'dd MMMM yyyy, HH:mm', {
                    locale: localeId,
                  })}
                </span>
              </div>
            ...
            <CardContent className="space-y-4">
              ...
              <div className="text-center p-4 bg-primary/10 rounded-lg">
                ...
                <div className="flex items-center justify-center gap-3">
                  ...
                  <Badge className={`text-lg ${getGradeColor(overallGrade.grade)}`}>
                    {overallGrade.grade}
                  </Badge>
                </div>
                <p className="text-sm text-muted-foreground mt-2">{overallGrade.label}</p>
              </div>
          ...
          <Card>
            ...
            <CardContent>
              <ul className="text-sm text-muted-foreground space-y-1">
                <li><strong>Kelancaran:</strong> Tidak banyak jeda, alur lancar</li>
                <li><strong>Tata Bahasa:</strong> Struktur kalimat tepat</li>
                <li><strong>Kosa Kata:</strong> Kata-kata beragam & tepat</li>
                <li><strong>Pengucapan:</strong> Jelas dan mudah dipahami</li>
              </ul>
            </CardContent>
          </Card>
        </div>
