...
// Types
export type RewardCategory = 'ACADEMIC' | 'RELIGIOUS' | 'EXTRACURRICULAR' | 'SOCIAL' | 'OTHER';
...
// Constants
export const REWARD_CATEGORIES: { value: RewardCategory; label: string; color: string }[] = [
  { value: 'ACADEMIC', label: 'Akademik', color: 'bg-blue-100 text-blue-800' },
  { value: 'RELIGIOUS', label: 'Keagamaan', color: 'bg-green-100 text-green-800' },
  { value: 'EXTRACURRICULAR', label: 'Ekstrakurikuler', color: 'bg-purple-100 text-purple-800' },
  { value: 'SOCIAL', label: 'Sosial', color: 'bg-orange-100 text-orange-800' },
  { value: 'OTHER', label: 'Lainnya', color: 'bg-gray-100 text-gray-800' },
];

// Reward Types Hooks
export function useRewardTypes(params?: { category?: RewardCategory; isActive?: boolean }) {
  return useQuery({
...
export function useRewardSummary(params?: { startDate?: string; endDate?: string }) {
  return useQuery({
...
export function useStudentRewardPoints(studentId: string) {
  return useQuery({
    ...
    queryFn: async () => {
      const response = await api.get<{ totalPoints: number; rewards: Reward[] }>(
