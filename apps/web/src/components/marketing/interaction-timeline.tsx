...
export function InteractionTimeline({ registrantId }: { registrantId: string }) {
      ...
      <CardContent>
        <div className="space-y-4">
            ...
            <div key={interaction.id} className="flex gap-4 p-4 border rounded-lg">
              <div className="bg-muted h-10 w-10 rounded-full flex items-center justify-center shrink-0">
                {interaction.type === 'CALL' ? <Phone className="h-5 w-5" /> :
                 interaction.type === 'WA' ? <MessageCircle className="h-5 w-5" /> :
              ...
              <div className="flex-1">
                <div className="flex justify-between items-start">
                  ...
                  <span className="text-xs text-muted-foreground">
                    {format(new Date(interaction.date), 'd MMM HH:mm', { locale: idLocale })}
                  </span>
                </div>
                {interaction.notes && (
                ...
                {interaction.nextActionDate && (
                  <p className="mt-2 text-xs text-blue-600 font-medium">
                    Jadwal Berikutnya: {format(new Date(interaction.nextActionDate), 'd MMM yyyy', { locale: idLocale })}
                  </p>
                )}
              </div>
            </div>
          ...
          {(!interactions || interactions.length === 0) && (
            <p className="text-center text-muted-foreground py-4">Belum ada interaksi</p>
          )}
        </div>
      </CardContent>
    </Card>
