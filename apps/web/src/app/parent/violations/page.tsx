...
export default function ViolationsPage() {
  ...
  useEffect(() => {
    ...
    const fetchViolations = async () => {
      setLoading(true);
      try {
        const res = await api.get(`/parent/children/${selectedChild}/violations`);
        ...
        <>
          ...
          {summary && (
            <div className="grid gap-4 md:grid-cols-3">
              ...
              <Card>
                <CardContent className="p-4 text-center">
                  <div className="text-2xl font-bold text-red-600">{summary.totalPoints}</div>
              ...
              <Card>
                <CardContent className="p-4 text-center">
                  <div className="text-lg font-bold">
                    {summary.totalPoints < 10 ? 'Baik' : 
                     summary.totalPoints < 25 ? 'Perlu Perhatian' : 
                     summary.totalPoints < 50 ? 'Peringatan' : 'Serius'}
                  </div>
            ...
            <CardContent>
              {violations.length === 0 ? (
                <div className="text-center py-8">
                  ...
                  <h3 className="text-lg font-medium text-green-600">Tidak ada pelanggaran</h3>
                        ...
                        <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
                          <div className="flex-1">
                            <div className="flex items-center gap-2">
                              <Badge variant={getTypeBadgeVariant(violation.type) as any}>
                                {getTypeLabel(violation.type)}
                              </Badge>
                              <Badge variant="outline">{violation.points} Poin</Badge>
                            </div>
                          ...
                          <div className="text-right text-sm text-muted-foreground">
                            <div className="flex items-center gap-1 justify-end">
                              ...
                              {new Date(violation.occurredAt).toLocaleDateString('id-ID', {
                                day: 'numeric',
                                month: 'short',
                                year: 'numeric',
                              })}
