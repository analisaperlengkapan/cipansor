...
export function useCorrespondence(unitId?: string) {
  ...
  const useLetters = (params: Omit<GetLettersParams, 'unitId'>) => {
    return useQuery({
      ...
      queryFn: async () => {
        if (!unitId) return { data: [], meta: { total: 0, page: 1, limit: 10, totalPages: 0 } };
  ...
  const useLetter = (id: string) => {
    return useQuery({
      queryKey: ['letter', id],
      queryFn: async () => {
        const response = await api.get<{ success: boolean; data: LetterDetail }>(`/correspondence/letters/${id}`);
  ...
  const reviewLetter = useMutation({
    mutationFn: async ({ id, action, notes }: { id: string; action: 'APPROVE' | 'REJECT'; notes?: string }) => {
      const response = await api.post(`/correspondence/letters/${id}/review`, { action, notes });
      return response.data;
    },
