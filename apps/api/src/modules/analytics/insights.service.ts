...
async function calculateAcademicRisk(studentId: string): Promise<RiskFactor> {
  ...
  const avgScore =
    grades.length > 0 ? grades.reduce((sum, g) => sum + Number(g.score), 0) / grades.length : 70;
...
async function calculateTahfidzRisk(studentId: string): Promise<RiskFactor> {
  ...
  if (!isActive) {
    score = 50;
    level = 'medium';
  } else if (recentProgress >= 100) {
    score = 0;
    level = 'low';
  } else if (recentProgress >= 50) {
    score = 20;
    level = 'low';
  } else {
    score = 40;
...
export async function getStudentsAtRisk(
  ...
  const assessments = await Promise.all(students.map((s) => calculateStudentRisk(s.id)));
...
export async function getClassRiskSummary(classId: string): Promise<{
  ...
  return {
    ...
    topRisks: assessments
      .filter((a) => a.overallRisk === 'high' || a.overallRisk === 'critical')
      .slice(0, 5),
  };
}
