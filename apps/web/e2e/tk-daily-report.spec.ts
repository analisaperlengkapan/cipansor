...
test.describe('TK Daily Report', () => {
    test.beforeEach(async ({ page }) => {
        ...
        await loginPage.loginAndWaitForDashboard('superadmin@cipansor.id', 'SuperAdmin123!');
    });
    ...
    test('should create daily report with photo', async ({ page }) => {
        ...
        await expect(page.getByRole('heading', { name: /buat laporan harian/i })).toBeVisible();

        // 1. Select Class
        const classSelect = page.locator('button[role="combobox"]').filter({ hasText: /pilih kelas/i });
        ...
        if (await classSelect.isVisible({ timeout: 5000 }).catch(() => false)) {
            ...
            // 2. Select Student
            const studentSelect = page.locator('button[role="combobox"]').filter({ hasText: /pilih siswa/i });
            ...
            // 3. Fill Mood
            const moodSelect = page.locator('button[role="combobox"]').filter({ hasText: /pilih mood/i });
            ...
            await expect(page.locator('img[alt*="Foto kegiatan"]').first()).toBeVisible({ timeout: 5000 });
