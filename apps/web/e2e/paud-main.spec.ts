...
test.describe('PAUD - Main Page', () => {
  ...
  test('should display PAUD overview with stats', async ({ page }) => {
    ...
    // Check for heading
    const heading = await page.getByRole('heading', { name: /PAUD|TK/i })
  ...
  test('should display menu cards for PAUD features', async ({ page }) => {
    ...
    const hasLinks = await page.locator('a[href*="/paud/"]')
      .first()
      .isVisible({ timeout: 5000 }).catch(() => false);
...
test.describe('PAUD - Tabs Navigation', () => {
  test('should switch between tabs', async ({ page }) => {
    ...
    const assessmentTab = page.getByRole('tab', { name: /penilaian|assessment/i });
  ...
  test('should display quick actions', async ({ page }) => {
    ...
    const hasQuickActions = await page.getByRole('button', { name: /buat|laporan|lihat/i })
      .first()
      .isVisible({ timeout: 5000 }).catch(() => false);
