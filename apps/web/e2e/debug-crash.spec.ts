    ...
    await page.route("**/api/**", async (route) => {
      ...
      if (url.includes("/api/foundation"))
        return route.fulfill({
          status: 200,
          body: JSON.stringify(
            apiResponse({ id: "f1", name: "Yayasan Pesantren Cipansor" }),
          ),
        });
      ...
      if (url.includes("/api/hr/departments"))
        return route.fulfill({
          status: 200,
          body: JSON.stringify(apiResponse([{ id: "d1", name: "Kurikulum" }])),
        });
      ...
      if (url.includes("/api/paud-assessment/indicators"))
        return route.fulfill({
          status: 200,
          body: JSON.stringify(
            apiResponse([{ id: "i1", name: "Indikator 1", aspect: "NAM" }]),
          ),
        });
      ...
      if (url.includes("/api/ibadah/stats/student"))
        return route.fulfill({
          status: 200,
          body: JSON.stringify(apiResponse({ totalPoints: 100 })),
        });
      ...
      if (url.includes("/api/violations/summary"))
        return route.fulfill({
          status: 200,
          body: JSON.stringify({
            totalViolations: 12,
            byCategory: [{ category: "LIGHT", count: 10 }],
          }),
        });
      ...
      if (url.includes("/api/violations/types"))
        return route.fulfill({
          status: 200,
          body: JSON.stringify(
            apiResponse([
      ...
      if (url.includes("/api/classes"))
        return route.fulfill({
          status: 200,
          body: JSON.stringify(paginated([{ id: "c1", name: "X-1" }])),
        });
