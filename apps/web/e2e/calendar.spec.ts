...
test.describe('Calendar - Features', () => {
  test('should display calendar view', async ({ page }) => {
    ...
    const hasCalendar = content.includes('Calendar') || 
                        ...
                        await page.locator('[class*="calendar"]').first().isVisible({ timeout: 5000 }).catch(() => false);
  ...
  test('should have event management', async ({ page }) => {
    ...
    const hasEventButton = await page.locator('button:has-text("Event"), button:has-text("Acara"), button:has-text("Tambah")')
      .first()
  ...
  test('should show month/week views', async ({ page }) => {
    ...
    const content = await page.content();
    const hasViewSelector = content.includes('Month') || 
