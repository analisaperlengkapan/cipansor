...
export default function CreateProcurementPage() {
  ...
  const [items, setItems] = useState<CreatePurchaseRequestItemInput[]>([
    { itemName: '', quantity: 1, unit: 'pcs', estimatedPrice: 0, assetCategoryId: undefined }
  ]);
  ...
  const handleItemChange = (index: number, field: keyof CreatePurchaseRequestItemInput, value: any) => {
      ...
      <div className="space-y-6 max-w-4xl mx-auto">
        <div className="flex justify-between items-center">
          <div>
            ...
            <p className="text-muted-foreground">Isi formulir pengajuan pengadaan barang.</p>
          </div>
        </div>
              ...
              {items.map((item, index) => (
                <div key={index} className="grid grid-cols-12 gap-4 items-end border-b pb-4 last:border-0 last:pb-0">
                  <div className="col-span-4 space-y-2">
                    <Label className={index > 0 ? 'sr-only' : ''}>Nama Barang</Label>
                    <Input
                      value={item.itemName}
                  ...
                  <div className="col-span-2 space-y-2">
                    <Label className={index > 0 ? 'sr-only' : ''}>Kategori Aset</Label>
                     <Select
                        value={item.assetCategoryId || 'none'}
                        onValueChange={(val) => handleItemChange(index, 'assetCategoryId', val === 'none' ? undefined : val)}
                      >
                        <SelectTrigger>
                  ...
                  <div className="col-span-3 space-y-2">
                    <Label className={index > 0 ? 'sr-only' : ''}>Est. Harga Satuan</Label>
                    <Input
                      type="number"
                      min="0"
