...
import { Loader2, User, BookOpen, Target, CheckCircle, AlertCircle } from 'lucide-react';
...
export default function HalaqohPage() {
  ...
  const { data: halaqohs, isLoading: isLoadingHalaqohs } = useQuery({
    ...
    queryFn: async () => {
      const res = await api.get('/takhosus/halaqoh', { params: { limit: 100 } });
  ...
  return (
    <div className="container mx-auto py-8 space-y-8">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-3xl font-bold tracking-tight">Manajemen Halaqoh</h1>
          <p className="text-muted-foreground mt-2">
            Pantau perkembangan santri dan target hafalan per kelompok.
          </p>
        </div>
        <div className="flex gap-2">
            <Button variant="outline" onClick={() => router.push('/takhosus/targets')}>
              ...
              <TableBody>
                {enrollments?.map((enrollment: any) => (
                  <TableRow key={enrollment.id}>
                    <TableCell>
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-muted-foreground">{enrollment.student.unit.name}</p>
                        </div>
                      </div>
                    </TableCell>
                    <TableCell>
                        <Badge variant="outline">{enrollment.halaqoh?.name || 'No Halaqoh'}</Badge>
                    </TableCell>
                    <TableCell>
                    ...
                    <TableCell>
                        <Button variant="ghost" size="sm" onClick={() => router.push(`/student/${enrollment.student.id}/takhosus`)}>
                            Detail
                        </Button>
                    </TableCell>
                  </TableRow>
