...
export function useStaffDashboardStats() {
  ...
  return useQuery({
    ...
    queryFn: async (): Promise<StaffDashboardStats> => {
      ...
      const [permitsRes, healthRes, violationsRes, rewardsRes, attendanceRes, studentsRes] = await Promise.all([
        // Pending permits
        api.get<PaginatedResponse<unknown>>('/permits', {
        ...
        api.get<PaginatedResponse<unknown>>('/health', {
          params: { status: 'ACTIVE', unitId: user?.unitId, limit: 1 }
        }).catch(() => ({ data: { meta: { pagination: { total: 0 } } } })),

        // Today's violations
        api.get<PaginatedResponse<unknown>>('/violations', {
          params: { date: today, unitId: user?.unitId, limit: 1 }
        }).catch(() => ({ data: { meta: { pagination: { total: 0 } } } })),

        // Today's rewards
        api.get<PaginatedResponse<unknown>>('/rewards', {
          params: { date: today, unitId: user?.unitId, limit: 1 }
        }).catch(() => ({ data: { meta: { pagination: { total: 0 } } } })),

        // Today's attendance summary
        api.get<ApiResponse<{ present: number; absent: number; sick: number; excused: number; total: number }>>('/attendance/summary', {
          params: { date: today, unitId: user?.unitId }
        }).catch(() => ({ data: { data: { present: 0, absent: 0, sick: 0, excused: 0, total: 0 } } })),

        // Active students count
        api.get<PaginatedResponse<unknown>>('/students', {
          params: { status: 'ACTIVE', unitId: user?.unitId, limit: 1 }
        }).catch(() => ({ data: { meta: { pagination: { total: 0 } } } })),
      ]);
      ...
      return {
        ...
        todayViolations: (violationsRes.data as any)?.meta?.pagination?.total || 0,
        ...
        attendanceToday: (attendanceRes.data as any)?.data || { present: 0, absent: 0, sick: 0, excused: 0, total: 0 },
      };
    },
    enabled: !!user?.unitId,
...
export function useStaffPendingTasks(limit: number = 10) {
  ...
  return useQuery({
    ...
    queryFn: async (): Promise<PendingTask[]> => {
      ...
      const permitsRes = await api.get<PaginatedResponse<{
        id: string;
        student?: { name: string };
        permitType: string;
        reason: string;
      ...
      const healthRes = await api.get<PaginatedResponse<{
        id: string;
        student?: { name: string };
        recordType: string;
        complaint: string;
...
export function useStaffRecentActivity(limit: number = 10) {
  ...
  return useQuery({
    ...
    queryFn: async (): Promise<RecentActivity[]> => {
      ...
      const violationsRes = await api.get<PaginatedResponse<{
        id: string;
        student?: { name: string };
      ...
      const rewardsRes = await api.get<PaginatedResponse<{
        id: string;
        student?: { name: string };
...
export function useApprovePermit() {
  ...
  return useMutation({
    mutationFn: async (permitId: string) => {
      const response = await api.post<ApiResponse<unknown>>(`/permits/${permitId}/approve`);
...
export function useRejectPermit() {
  ...
  return useMutation({
    mutationFn: async ({ permitId, reason }: { permitId: string; reason: string }) => {
      const response = await api.post<ApiResponse<unknown>>(`/permits/${permitId}/reject`, { reason });
      return response.data;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['staff-dashboard-stats'] });
...
export function useStaffDashboard() {
  ...
  return {
    ...
    isLoading: stats.isLoading || pendingTasks.isLoading || recentActivity.isLoading,
