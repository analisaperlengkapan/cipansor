...
export function useSubmitAssignment() {
  ...
  return useMutation({
    mutationFn: async ({ id, data }: { id: string; data: SubmitAssignmentRequest }) => {
      const response = await api.post<ApiResponse<AssignmentSubmission>>(`/assignments/${id}/submit`, data);
      return response.data.data;
    },
    onSuccess: (_, variables) => {
      queryClient.invalidateQueries({ queryKey: ['assignments', variables.id] });
...
export function useGradeSubmission() {
  ...
  return useMutation({
    mutationFn: async ({ assignmentId, studentId, data }: { assignmentId: string; studentId: string; data: GradeSubmissionRequest }) => {
      const response = await api.post<ApiResponse<AssignmentSubmission>>(`/assignments/${assignmentId}/submissions/${studentId}/grade`, data);
      return response.data.data;
    },
    onSuccess: (_, variables) => {
      queryClient.invalidateQueries({ queryKey: ['assignments', variables.assignmentId, 'submissions'] });
    },
  });
}
