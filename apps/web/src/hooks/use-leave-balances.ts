...
export function useInitializeLeaveBalance() {
  ...
  return useMutation({
    mutationFn: async (data: { userId: string; academicYearId: string; leaveType: string; totalDays: number }) => {
      const response = await api.post('/hr/leave-balances/initialize', data);
      return response.data.data;
    },
    onSuccess: (_, variables) => {
      queryClient.invalidateQueries({ queryKey: ['leave-balances', variables.userId] });
    },
  });
}
...
export function useUpdateLeaveBalance() {
  ...
  return useMutation({
    mutationFn: async ({ id, totalDays, userId }: { id: string; totalDays: number; userId: string }) => {
      const response = await api.patch(`/hr/leave-balances/${id}`, { totalDays });
      return response.data.data;
    },
    onSuccess: (_, variables) => {
      queryClient.invalidateQueries({ queryKey: ['leave-balances', variables.userId] });
