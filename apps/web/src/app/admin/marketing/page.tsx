...
export default function MarketingDashboardPage() {
  ...
  // Calculate totals
  const totalRegistrants = stats?.sources.reduce((acc: number, curr: any) => acc + curr.count, 0) || 0;
  const topSource = stats?.sources.sort((a: any, b: any) => b.count - a.count)[0]?.source || '-';
        ...
        <div className="grid gap-4 md:grid-cols-3">
          <Card>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-medium">Total Registrants (Tracked)</CardTitle>
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-7">
          ...
          <Card className="col-span-3">
            <CardHeader>
              ...
              <CardDescription>
                Performance by registrant count
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div className="space-y-4">
                {stats?.topCampaigns.map((campaign: any, index: number) => (
                  <div key={index} className="flex items-center">
                    ...
                    <div className="ml-auto font-medium">{campaign.registrants} Leads</div>
                  </div>
                ))}
              </div>
            </CardContent>
