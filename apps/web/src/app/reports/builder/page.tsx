...
// Types
type ReportType = 'students' | 'attendance' | 'finance' | 'tahfidz' | 'academic' | 'teachers';
...
const REPORT_TYPES: Array<{ type: ReportType; label: string; description: string; icon: React.ElementType }> = [
    { type: 'students', label: 'Data Santri', description: 'Daftar santri lengkap', icon: Users },
    { type: 'attendance', label: 'Kehadiran', description: 'Rekap kehadiran', icon: UserCheck },
    { type: 'finance', label: 'Keuangan', description: 'Tagihan & pembayaran', icon: DollarSign },
    { type: 'tahfidz', label: 'Tahfidz', description: 'Hafalan Al-Quran', icon: BookOpen },
    { type: 'academic', label: 'Akademik', description: 'Nilai & prestasi', icon: GraduationCap },
    { type: 'teachers', label: 'Guru', description: 'Data pengajar', icon: UserCog },
];
...
const REPORT_FIELDS: Record<ReportType, ReportField[]> = {
    ...
    finance: [
        ...
        { key: 'paidAmount', label: 'Terbayar', type: 'number', category: 'Pembayaran' },
        { key: 'status', label: 'Status', type: 'string', category: 'Status' },
    ...
    tahfidz: [
        ...
        { key: 'totalAyah', label: 'Total Ayat', type: 'number', category: 'Hafalan' },
...
export default function ReportBuilderPage() {
    ...
    const generateMutation = useMutation({
        mutationFn: async () => {
            ...
            const rows = Array.from({ length: Math.min(config.limit, 10) }).map((_, i) => {
        ...
        <MainLayout>
            <div className="space-y-6">
                ...
                <div className="flex items-center gap-2">
                    <Badge variant={step === 'type' ? 'default' : 'secondary'}>1. Pilih Tipe</Badge>
                    <div className="h-px w-8 bg-border" />
                    <Badge variant={step === 'fields' ? 'default' : 'secondary'}>2. Pilih Field</Badge>
                    <div className="h-px w-8 bg-border" />
                    ...
                    <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
                        {REPORT_TYPES.map((report) => {
                            ...
                            return (
                                <Card
                                    key={report.type}
                                    className={`cursor-pointer transition-all hover:border-primary ${config.type === report.type ? 'border-primary bg-primary/5' : ''
                                        }`}
                            ...
                            <CardContent>
                                <div className="space-y-6">
                                    {Object.entries(fieldsByCategory).map(([category, fields]) => (
                                        <div key={category}>
                                            <h4 className="mb-3 font-medium text-muted-foreground">{category}</h4>
                            ...
                            <CardContent className="space-y-4">
                                ...
                                <div>
                                    ...
                                    <Select
                                        ...
                                        onValueChange={(v) => setConfig((prev) => ({ ...prev, limit: Number(v) }))}
                                    >
                                        <SelectTrigger>
                                            <SelectValue />
                                        </SelectTrigger>
                                ...
                                <div className="flex gap-2">
                                    ...
                                    <Button
                                        ...
                                        disabled={config.fields.length === 0 || generateMutation.isPending}
                                    >
                        ...
                        <CardHeader>
                            <div className="flex items-center justify-between">
                                <div>
                                    ...
                                    <CardDescription>{reportData.length} baris data</CardDescription>
                                </div>
                                <div className="flex gap-2">
                                    ...
                                    <TableHeader>
                                        <TableRow>
                                            {config.fields.map((fieldKey) => {
                                                ...
                                                return <TableHead key={fieldKey}>{field?.label || fieldKey}</TableHead>;
                                            })}
                                        </TableRow>
                                    </TableHeader>
                                    <TableBody>
                                        {reportData.map((row, idx) => (
                                            <TableRow key={idx}>
                                                {config.fields.map((fieldKey) => (
                                                    <TableCell key={fieldKey}>{String(row[fieldKey] ?? '')}</TableCell>
                                                ))}
                                            </TableRow>
                                        ))}
                                    </TableBody>
